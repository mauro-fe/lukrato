...
class ComparativesService
{
    ...
    private function queryPeriodTotals(Carbon $start, Carbon $end): object
    {
        $query = Lancamento::where('user_id', $this->userId)
            ...
            ->where('pago', 1)
            ->where('afeta_caixa', 1);
        $despesaCase = $this->despesaSemPagamentoFaturaCaseSql();
    ...
    private function despesaSemPagamentoFaturaCaseSql(): string
    {
        return 'WHEN tipo = "despesa"
