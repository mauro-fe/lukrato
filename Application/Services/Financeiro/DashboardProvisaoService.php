...
class DashboardProvisaoService
{
    ...
    public function calcularSaldoConta(int $userId, int $contaId, string $ate): float
    {
        ...
        $transfIn  = (float) Lancamento::where('user_id', $userId)->where('pago', 1)->where('afeta_caixa', 1)->where('data', '<=', $ate)->where('eh_transferencia', 1)->where('conta_id_destino', $contaId)->sum('valor');
        $transfOut = (float) Lancamento::where('user_id', $userId)->where('pago', 1)->where('afeta_caixa', 1)->where('data', '<=', $ate)->where('eh_transferencia', 1)->where('conta_id', $contaId)->sum('valor');
    ...
    public function calcularSaldoGlobal(int $userId, string $ate): float
    {
        ...
        $base = Lancamento::where('user_id', $userId)->where('pago', 1)->where('afeta_caixa', 1)->where('data', '<=', $ate)->where('eh_transferencia', 0);
