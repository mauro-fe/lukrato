...
class MetaService
{
    ...
    public function atualizar(int $userId, int $id, array $data): ?array
    {
        ...
        foreach (['titulo', 'descricao', 'tipo', 'modelo', 'valor_alvo', 'data_inicio', 'data_prazo', 'icone', 'cor', 'prioridade', 'status'] as $field) {
            if (array_key_exists($field, $data)) {
                $updateData[$field] = $this->normalizeUpdateValue($field, $data[$field]);
            }
        }
    ...
    private function normalizeUpdateValue(string $field, mixed $value): mixed
    {
        if ($field === 'valor_alvo') {
            $valorAlvo = round((float) $value, 2);
            if ($valorAlvo <= 0) {
