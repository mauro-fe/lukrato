...
<section class="container">
    ...
    <div class="c-card mt-4">
        <form id="formNova" class="row" style="display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:var(--spacing-3);margin:var(--spacing-4) 0;">
      ...
      <button class="lk-btn danger btn-del" data-del="${cat.id}" title="Excluir" aria-label="Excluir">
        ...
        async function load() {
            ...
            try {
                ...
                if ((j.data || []).length === 0) {
                    const tr = document.createElement('tr');
                    tr.innerHTML = `<td colspan="4" class="c-muted" style="padding:var(--spacing-4)">Nenhuma categoria encontrada.</td>`;
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await load(); // garante atualização imediata
            } catch (e) {
                alertError(e.message);
            }
        });
        ...
        tbody.addEventListener('click', async (e) => {
            ...
            try {
                let j;
                try {
                    // rota preferida: api/categorias/{id}/delete
                    j = await tryFetch(`categorias/${id}/delete`, {
                        method: 'POST'
                    });
                } catch (e1) {
                    // fallback: api/categorias/delete com id no corpo
