...
<script>
    ...
    `;
        ...
        // máscara saldo inicial
        fSaldo?.addEventListener('blur', () => {
            const v = parseMoneyBR(fSaldo.value);
            fSaldo.value = v ? formatMoneyBR(v) : '';
        });

        // ====== Modal de Lançamento por Conta (se existir no HTML) ======
        // IDs esperados no HTML:
        // #modalLancConta, #formLancConta, #lanContaId, #lanTipo, #lanData, #lanCategoria, #lanDescricao, #lanValor, #lanPago, #lanPagoLabel
        // ====== Modal de Lançamento por Conta ======
        ...
        // Helper: buscar elementos escopados ao modal
        const qM = (sel) => modalLanc ? modalLanc.querySelector(sel) : null;

        // Campos (todos escopados ao modal de contas)
