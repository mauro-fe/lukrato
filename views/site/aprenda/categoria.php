<?php
/**
 * Listagem por categoria — /aprenda/categoria/{slug}
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center max-w-2xl mx-auto" data-aos="fade-up">
            <div class="inline-flex items-center justify-center w-16 h-16 bg-orange-50 dark:bg-orange-900/20 rounded-2xl mb-4">
                <i data-lucide="<?= htmlspecialchars($categoria->icone ?? 'folder') ?>" class="w-8 h-8 text-primary" aria-hidden="true"></i>
            </div>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <article class="aprenda-post-card group" data-aos="fade-up" data-aos-delay="<?= $i * 80 ?>">
                    <a href="<?= rtrim(BASE_URL, '/') ?>/aprenda/<?= htmlspecialchars($post->slug) ?>" class="block">
                        ...
                        <div class="aprenda-post-body">
                            <h2 class="text-lg font-bold text-gray-900 dark:text-white group-hover:text-primary transition-colors line-clamp-2">
                            ...
                            <div class="aprenda-post-meta">
                                <?php if ($post->tempo_leitura): ?>
                                    <span><i data-lucide="clock" class="w-3.5 h-3.5" aria-hidden="true"></i> <?= $post->tempo_leitura ?> min</span>
                                <?php endif; ?>
        ...
        <?php if ($totalPages > 1): ?>
            <nav class="aprenda-pagination" aria-label="Paginação">
                <?php if ($page > 1): ?>
                    <a href="<?= rtrim(BASE_URL, '/') ?>/aprenda/categoria/<?= $categoria->slug ?>?page=<?= $page - 1 ?>" class="aprenda-page-btn" aria-label="Página anterior">
                ...
                if ($start > 1) {
                    echo '<a href="' . rtrim(BASE_URL, '/') . '/aprenda/categoria/' . $categoria->slug . '?page=1" class="aprenda-page-btn">1</a>';
                    if ($start > 2) echo '<span class="aprenda-page-dots">…</span>';
                }
                for ($p = $start; $p <= $end; $p++) {
                    $active = $p === $page ? ' aprenda-page-active' : '';
                    echo '<a href="' . rtrim(BASE_URL, '/') . '/aprenda/categoria/' . $categoria->slug . '?page=' . $p . '" class="aprenda-page-btn' . $active . '">' . $p . '</a>';
                }
                if ($end < $totalPages) {
                    if ($end < $totalPages - 1) echo '<span class="aprenda-page-dots">…</span>';
                    echo '<a href="' . rtrim(BASE_URL, '/') . '/aprenda/categoria/' . $categoria->slug . '?page=' . $totalPages . '" class="aprenda-page-btn">' . $totalPages . '</a>';
                }
                ?>

                <?php if ($page < $totalPages): ?>
                    <a href="<?= rtrim(BASE_URL, '/') ?>/aprenda/categoria/<?= $categoria->slug ?>?page=<?= $page + 1 ?>" class="aprenda-page-btn" aria-label="Próxima página">
...
<?php else: ?>
    <section class="py-20 bg-gray-50 dark:bg-gray-800/50">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <a href="<?= rtrim(BASE_URL, '/') ?>/aprenda" class="inline-flex items-center gap-2 text-primary font-medium hover:underline">
