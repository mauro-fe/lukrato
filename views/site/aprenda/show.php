 ...
 *
 * Variáveis: $post, $relacionados, $relatedLayout, $breadcrumbItems
 */
?>
...
<script type="application/ld+json">
    ...
    echo json_encode(
        array_filter([
            ...
            'mainEntityOfPage' => [
                '@type' => 'WebPage',
                '@id' => rtrim(BASE_URL, '/') . '/blog/' . $post->slug,
            ],
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-3">
        <ol class="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 flex-wrap">
            <?php foreach ($breadcrumbItems as $i => $item): ?>
                ...
                <?php else: ?>
                    <li class="text-gray-900 dark:text-white font-medium truncate max-w-[200px] sm:max-w-none">
...
<?php $isSidebar = ($relatedLayout ?? 'bottom') === 'sidebar' && count($relacionados) > 0; ?>

<!-- Artigo -->
<article class="py-12 bg-white dark:bg-gray-900">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="<?= $isSidebar ? 'aprenda-article-layout' : 'max-w-3xl mx-auto' ?>">
            <?php if ($isSidebar): ?><div class="aprenda-article-main"><?php endif; ?>

                <!-- Header -->
                <header class="mb-8" data-aos="fade-up">
                    <?php if ($post->categoria): ?>
                        <a href="<?= rtrim(BASE_URL, '/') ?>/blog/categoria/<?= htmlspecialchars($post->categoria->slug) ?>"
                    ...
                    <div
                        class="flex items-center gap-4 text-sm text-gray-500 dark:text-gray-400 pb-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="mt-12 pt-8 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex items-center gap-3">
                        <?php
                        $shareUrl = urlencode(rtrim(BASE_URL, '/') . '/blog/' . $post->slug);
                        $shareTitle = urlencode($post->titulo);
                        ?>
                        <a href="https://wa.me/?text=<?= $shareTitle ?>%20<?= $shareUrl ?>" target="_blank"
                            rel="noopener noreferrer" class="aprenda-share-btn" aria-label="Compartilhar no WhatsApp"
                            title="WhatsApp">
                        ...
                        <a href="https://twitter.com/intent/tweet?text=<?= $shareTitle ?>&url=<?= $shareUrl ?>"
                            target="_blank" rel="noopener noreferrer" class="aprenda-share-btn"
                        ...
                        <a href="https://www.linkedin.com/sharing/share-offsite/?url=<?= $shareUrl ?>" target="_blank"
                            rel="noopener noreferrer" class="aprenda-share-btn" aria-label="Compartilhar no LinkedIn"
                            title="LinkedIn">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                <path
            ...
            <?php if ($isSidebar): ?>
                <!-- Sidebar: Relacionados (variante lateral) -->
                <aside class="aprenda-sidebar" aria-label="Artigos relacionados">
                    <div class="aprenda-sidebar-inner">
                        <h2 class="aprenda-sidebar-title">
                            <i data-lucide="bookmark" class="w-4 h-4" aria-hidden="true"></i>
                            Leia também
                        </h2>
                        <div class="aprenda-sidebar-list">
                            <?php foreach ($relacionados as $i => $rel): ?>
                                <a href="<?= rtrim(BASE_URL, '/') ?>/blog/<?= htmlspecialchars($rel->slug) ?>"
                                    ...
                                    <?php endif; ?>
                                    <span class="aprenda-sidebar-item-title"><?= htmlspecialchars($rel->titulo) ?></span>
                                </a>
                            <?php endforeach; ?>
                        </div>
                    </div>
...
<!-- Relacionados (variante bottom) -->
<?php if (!$isSidebar && count($relacionados) > 0): ?>
    <section class="py-16 bg-gray-50 dark:bg-gray-800/50" id="related-bottom-section">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
                <?php foreach ($relacionados as $i => $rel): ?>
                    <article class="aprenda-post-card group" data-aos="fade-up" data-aos-delay="<?= $i * 100 ?>">
                        <a href="<?= rtrim(BASE_URL, '/') ?>/blog/<?= htmlspecialchars($rel->slug) ?>" class="block"
                            data-track="related-click" data-variant="bottom" data-position="<?= $i ?>">
                            <?php if ($rel->imagem_capa): ?>
                                <div class="aprenda-post-img">
                                    <img src="<?= rtrim(BASE_URL, '/') ?>/<?= htmlspecialchars($rel->imagem_capa) ?>"
                                        alt="<?= htmlspecialchars($rel->titulo) ?>" loading="lazy" width="400" height="225">
                                </div>
                            <?php else: ?>
...
<!-- Tracking: related clicks + scroll depth -->
<script>
    (function() {
        // Track clicks on related post links
        document.querySelectorAll('[data-track="related-click"]').forEach(function(link) {
            link.addEventListener('click', function() {
                var variant = this.getAttribute('data-variant') || 'unknown';
                var position = this.getAttribute('data-position') || '0';
                if (typeof gtag === 'function') {
                    gtag('event', 'related_post_click', {
                        event_category: 'blog_engagement',
                        event_label: variant + '_pos' + position,
                        variant: variant,
        ...
        // Scroll depth: observe when bottom related section enters viewport
        var bottomSection = document.getElementById('related-bottom-section');
        if (bottomSection && typeof IntersectionObserver !== 'undefined') {
            var observer = new IntersectionObserver(function(entries) {
                entries.forEach(function(entry) {
                    if (entry.isIntersecting) {
                        if (typeof gtag === 'function') {
                            gtag('event', 'related_section_viewed', {
                                event_category: 'blog_scroll_depth',
                                event_label: 'bottom'
                            });
                        }
                        observer.disconnect();
