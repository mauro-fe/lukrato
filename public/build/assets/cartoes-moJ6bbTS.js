...
`)},setupLimiteMoneyMask(){const e=document.getElementById("limiteTotal");if(!e){console.error("❌ Campo limiteTotal NÃO encontrado!");return}e.addEventListener("input",function(a){let t=a.target.value;t=t.replace(/[^\d]/g,"");const n=((parseInt(t)||0)/100).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".");a.target.value=n}),e.value="0,00"}};function J(e){return typeof e!="string"?e:e.startsWith(f.BASE_URL)?e.slice(f.BASE_URL.length):e}async function $(e,{method:a="GET",data:t=null,headers:o={},timeout:r=15e3}={}){return W(J(e),{method:a,headers:o,body:t},{timeout:r})}function Y(e){if(l.previewMeta=e?.is_demo?e:null,l.previewMeta){window.LKDemoPreviewBanner?.show(l.previewMeta);return}window.LKDemoPreviewBanner?.hide()}const S={async loadCartoes(){const e=document.getElementById("cartoesGrid"),a=document.getElementById("emptyState"),t=document.getElementById("cartoesContainer");if(!(!e||!a))try{l.isLoading=!0,e.setAttribute("aria-busy","true"),t?.setAttribute("aria-busy","true"),e.innerHTML=`
            ...
            `,I()}finally{l.isLoading=!1,e.setAttribute("aria-busy","false"),t?.setAttribute("aria-busy","false")}},async verificarFaturasPendentes(){l.cartoes.forEach(a=>{a.temFaturaPendente=!1});const e=l.cartoes.map(async a=>{try{const t=await $(`${f.API_URL}/cartoes/${a.id}/faturas-pendentes`),o=w(t,{}),r=o?.meses||w(o,[])||[];a.temFaturaPendente=Array.isArray(r)&&r.length>0}catch{a.temFaturaPendente=!1}});await Promise.all(e)},async carregarAlertas(){try{let e;if(window.lkFetch){const a=await window.lkFetch.get(`${f.API_URL}/cartoes/alertas`,{timeout:1e4,maxRetries:1,showLoading:!1});e=w(a,{});const t=w(e,{});l.alertas=t?.alertas||[]}else{e=await $(`${f.API_URL}/cartoes/alertas`,{timeout:1e4});const a=w(e,{});l.alertas=a?.alertas||[]}S.renderAlertas()}catch(e){j("[Cartoes] Erro ao carregar alertas",e,"Erro ao carregar alertas"),l.alertas=[];const a=document.getElementById("alertasContainer");a&&(a.style.display="none")}},renderAlertas(){const e=document.getElementById("alertasContainer");if(e){if(l.alertas.length===0){e.style.display="none";return}e.style.display="block",e.innerHTML=`
        ...
        `},dismissAlerta(e){const a=e.closest(".alerta-item");a&&(a.style.animation="slideOut 0.3s ease-out forwards",setTimeout(()=>{a.remove();const t=document.getElementById("alertasContainer");t&&t.querySelectorAll(".alerta-item").length===0&&(t.style.display="none")},300))},async loadContasSelect(){const e=document.getElementById("contaVinculada"),a=document.getElementById("contaVinculadaHelp"),t=document.getElementById("cartaoContaEmptyHint");if(!e){console.error("❌ Select contaVinculada não encontrado!");return}try{const o=`${f.API_URL}/contas?only_active=0&with_balances=1`,r=await $(o),n=w(r,{});let c=[];if(Array.isArray(n)?c=n:Array.isArray(n?.contas)&&(c=n.contas),c.length===0)return e.disabled=!0,e.innerHTML='<option value="">Nenhuma conta disponivel</option>',a&&(a.textContent="Crie uma conta antes de vincular um cartao."),t&&(t.hidden=!1),console.warn("⚠️ Nenhuma conta encontrada"),0;const u=c.map(i=>{const p=i.instituicao_financeira?.nome||i.instituicao?.nome||i.nome||"Sem instituição",C=s.escapeHtml(i.nome||"Conta sem nome"),y=s.escapeHtml(p),g=parseFloat(i.saldoAtual||i.saldo_atual||i.saldo||i.saldo_inicial||0),b=s.formatMoney(g);return`<option value="${i.id}">${C} - ${y} - ${b}</option>`}).join("");return e.disabled=!1,e.innerHTML='<option value="">Selecione a conta</option>'+u,a&&(a.textContent="Conta onde o pagamento da fatura sera debitado."),t&&(t.hidden=!0),c.length}catch(o){return B("[Cartoes] Erro ao carregar contas",o,"Erro ao carregar contas"),e.disabled=!0,e.innerHTML='<option value="">Erro ao carregar contas</option>',a&&(a.textContent="Nao foi possivel carregar as contas agora."),t&&(t.hidden=!1),0}},async saveCartao(){const e=document.getElementById("formCartao");if(!e.checkValidity()){e.reportValidity();return}const a=document.getElementById("cartaoId").value,t=!!a,o=document.querySelector('meta[name="csrf-token"]')?.content||document.querySelector('input[name="csrf_token"]')?.value||"",r=document.getElementById("limiteTotal").value,n=s.parseMoney(r),c=document.getElementById("cartaoLembreteAviso")?.value||"",u=document.getElementById("contaVinculada"),i=document.getElementById("cartaoCanalInapp"),p=document.getElementById("cartaoCanalEmail");if(u?.disabled){s.showToast("error","Crie uma conta antes de cadastrar um cartao.");return}if(c&&!i?.checked&&!p?.checked){s.showToast("error","Selecione pelo menos um canal para o lembrete.");return}const C={nome_cartao:document.getElementById("nomeCartao").value.trim(),conta_id:u?.value?parseInt(u.value,10):null,bandeira:document.getElementById("bandeira").value,ultimos_digitos:document.getElementById("ultimosDigitos").value.trim(),limite_total:n,dia_fechamento:document.getElementById("diaFechamento").value||null,dia_vencimento:document.getElementById("diaVencimento").value||null,lembrar_fatura_antes_segundos:c?parseInt(c):null,fatura_canal_inapp:c&&i?.checked?1:0,fatura_canal_email:c&&p?.checked?1:0,csrf_token:o};try{const y=t?`${f.API_URL}/cartoes/${a}`:`${f.API_URL}/cartoes`,g=await $(y,{method:t?"PUT":"POST",data:C}),b=w(g,null);b?.gamification?.achievements&&Array.isArray(b.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(b.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",t?"Cartão atualizado com sucesso!":"Cartão criado com sucesso!"),v.UI.closeModal(),await S.loadCartoes()}catch(y){B("[Cartoes] Erro ao salvar cartão",y,"Erro ao salvar cartão"),s.showToast("error",F(y,"Erro ao salvar cartao"))}},async editCartao(e){const a=l.cartoes.find(t=>t.id===e);if(a){if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para editar.");return}v.UI.openModal("edit",a)}},async arquivarCartao(e){const a=l.cartoes.find(o=>o.id===e);if(!a)return;if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para arquivar ou editar.");return}if(await s.showConfirmDialog("Arquivar Cartão",`Tem certeza que deseja arquivar o cartão "${a.nome_cartao}"? Você poderá restaurá-lo depois na página de Cartões Arquivados.`,"Arquivar"))try{await $(`${f.API_URL}/cartoes/${e}/archive`,{method:"POST"}),s.showToast("success","Cartão arquivado com sucesso!"),S.loadCartoes()}catch(o){B("[Cartoes] Erro ao arquivar cartão",o,"Erro ao arquivar cartão"),s.showToast("error",F(o,"Erro ao arquivar cartao"))}},async deleteCartao(e){return S.arquivarCartao(e)},async carregarFatura(e,a,t){try{const o=await $(`${f.API_URL}/cartoes/${e}/fatura?mes=${a}&ano=${t}`);return w(o,{itens:[],total:0,pago:0,pendente:0})}catch(o){if(o?.status===404)return{itens:[],total:0,pago:0,pendente:0};throw new Error(F(o,"Erro ao carregar fatura"))}},async carregarParcelamentosResumo(e,a,t){const o=await $(`${f.API_URL}/cartoes/${e}/parcelamentos-resumo?mes=${a}&ano=${t}`);return w(o,null)},async carregarHistoricoFaturas(e,a=12){const t=await $(`${f.API_URL}/cartoes/${e}/faturas-historico?limite=${a}`);return w(t,null)},async pagarParcelasIndividuais(e,a){try{const t=Array.from(e).map(n=>parseInt(n.dataset.id)),o=a.cartao_id||a.cartao?.id;if(!o)throw new Error("ID do cartão não encontrado na fatura");const r=await $(`${f.API_URL}/cartoes/${o}/parcelas/pagar`,{method:"POST",data:{parcela_ids:t,mes:a.mes,ano:a.ano}});if(r?.success!==!1){s.showToast("success",r.message||"Parcelas pagas com sucesso!");const n=document.querySelector(".modal-fatura-overlay");n&&v.Fatura.fecharModalFatura(n),await S.loadCartoes()}else throw new Error(r.message||"Erro ao pagar parcelas")}catch(t){s.showToast("error",F(t,"Erro ao processar a operacao do cartao"))}},async desfazerPagamento(e,a,t){if((await Swal.fire({title:"Desfazer pagamento?",html:`
            ...
            <article
                ...
                tabindex="0"
                role="button"
                aria-label="Abrir detalhes do cartao ${g}, ${T(r)} do limite usado"
            >
                ...
                <div class="card-header">
                    <div class="card-card-badges">
                        <span class="card-meta-chip card-meta-chip--brand" ${_(`Bandeira ${b}`,"Rede de processamento usada por este cartao para compras e parcelamentos.")}>
                            <i data-lucide="badge-check"></i>
                            ${b}
                        </span>
                        <span class="card-meta-chip card-meta-chip--status ${e.temFaturaPendente?"is-pending":"is-ok"}" ${_(h,e.temFaturaPendente?"Ha uma fatura aberta para este cartao que merece acompanhamento ou pagamento.":"Sem pendencias abertas para o ciclo atual deste cartao.")}>
        ...
        `,document.body.appendChild(o),I();const u=a?.target?.closest(".card-overflow-btn"),i=()=>{if(!u)return;const g=u.getBoundingClientRect(),b=o.offsetWidth||188,h=o.offsetHeight||156,E=Math.min(window.scrollX+window.innerWidth-b-12,Math.max(window.scrollX+12,g.right+window.scrollX-b)),L=Math.min(window.scrollY+window.innerHeight-h-12,g.bottom+window.scrollY+8);o.style.left=`${E}px`,o.style.top=`${L}px`};requestAnimationFrame(i),o.querySelectorAll("[data-card-menu-action]").forEach(g=>{g.addEventListener("click",b=>{switch(b.stopPropagation(),g.dataset.cardMenuAction){case"invoice":window.cartoesManager?.verFatura?.(e);break;case"edit":window.cartoesManager?.editCartao?.(e);break;case"archive":window.cartoesManager?.arquivarCartao?.(e);break}d.closeCardMenu()})});const p=g=>{!o.contains(g.target)&&!g.target.closest(".card-overflow-btn")&&d.closeCardMenu()},C=g=>{g.key==="Escape"&&d.closeCardMenu()},y=()=>i();document.addEventListener("click",p),document.addEventListener("keydown",C),window.addEventListener("resize",y),window.addEventListener("scroll",y,!0),d._cardMenuCleanup=()=>{document.removeEventListener("click",p),document.removeEventListener("keydown",C),window.removeEventListener("resize",y),window.removeEventListener("scroll",y,!0)}},async showCardDetails(e){const a=l.cartoes.find(t=>t.id===e);if(a){if(k(a)){U();return}if(window.LK_CardDetail?.open){window.LK_CardDetail.open(e,a.nome_cartao||a.nome||"Cartao",D(a),Z());return}v.Fatura?.verFatura?.(e)}},async exportarRelatorio(){if(!l.filteredCartoes?.length){typeof Swal<"u"&&Swal.fire({toast:!0,position:"top-end",icon:"info",title:"Nenhum cartao para exportar",text:"Adicione cartoes ou altere os filtros.",showConfirmButton:!1,timer:3e3,timerProgressBar:!0});return}try{const{jsPDF:e}=window.jspdf,a=new e,t=new Date,o=t.toLocaleDateString("pt-BR",{month:"long",year:"numeric"}),r=l.filteredCartoes.reduce((h,E)=>h+parseFloat(E.limite_total||0),0),n=l.filteredCartoes.reduce((h,E)=>h+parseFloat((E.limite_disponivel_real??E.limite_disponivel)||0),0),c=r-n,u=r>0?(c/r*100).toFixed(1):0,i=[230,126,34],p=[26,31,46],C=[248,249,250];a.setFillColor(...i),a.rect(0,0,210,35,"F"),a.setTextColor(255,255,255),a.setFontSize(22),a.setFont(void 0,"bold"),a.text("RELATORIO DE CARTOES DE CREDITO",105,15,{align:"center"}),a.setFontSize(10),a.setFont(void 0,"normal"),a.text(`Periodo: ${o}`,105,22,{align:"center"}),a.text(`Gerado em: ${t.toLocaleDateString("pt-BR")} as ${t.toLocaleTimeString("pt-BR")}`,105,28,{align:"center"});let y=45;a.setTextColor(...p),a.setFontSize(14),a.setFont(void 0,"bold"),a.text("RESUMO FINANCEIRO",14,y),y+=8,a.autoTable({startY:y,head:[["Indicador","Valor"]],body:[["Total de Cartoes",l.filteredCartoes.length.toString()],["Limite Total Combinado",s.formatMoney(r)],["Limite Utilizado",s.formatMoney(c)],["Limite Disponivel",s.formatMoney(n)],["Percentual de Utilizacao",`${u}%`]],theme:"grid",headStyles:{fillColor:i,textColor:[255,255,255],fontStyle:"bold",halign:"left"},columnStyles:{0:{cellWidth:100,fontStyle:"bold"},1:{cellWidth:86,halign:"right"}},styles:{fontSize:10,cellPadding:5},alternateRowStyles:{fillColor:C}}),y=a.lastAutoTable.finalY+15,a.setFontSize(14),a.setFont(void 0,"bold"),a.text("DETALHAMENTO POR CARTAO",14,y),y+=5;const g=l.filteredCartoes.map(h=>{const E=h.limite_disponivel_real??h.limite_disponivel??0,L=h.limite_total>0?((h.limite_total-E)/h.limite_total*100).toFixed(1):0;return[h.nome_cartao,s.formatBandeira(h.bandeira),`**** ${h.ultimos_digitos}`,s.formatMoney(h.limite_total),s.formatMoney(E),`${L}%`,h.ativo?"Ativo":"Inativo"]});a.autoTable({startY:y,head:[["Cartao","Bandeira","Final","Limite Total","Disponivel","Uso","Status"]],body:g,theme:"grid",headStyles:{fillColor:i,textColor:[255,255,255],fontStyle:"bold",halign:"center"},columnStyles:{0:{cellWidth:40},1:{cellWidth:25,halign:"center"},2:{cellWidth:25,halign:"center"},3:{cellWidth:28,halign:"right"},4:{cellWidth:28,halign:"right"},5:{cellWidth:18,halign:"center"},6:{cellWidth:22,halign:"center"}},styles:{fontSize:9,cellPadding:4},alternateRowStyles:{fillColor:C}});const b=a.internal.getNumberOfPages();for(let h=1;h<=b;h++)a.setPage(h),a.setFontSize(8),a.setTextColor(128,128,128),a.text(`Pagina ${h} de ${b} | Lukrato - Sistema de Gestao Financeira`,105,287,{align:"center"});a.save(`relatorio_cartoes_${t.toISOString().split("T")[0]}.pdf`),s.showToast("success","Relatorio exportado com sucesso")}catch(e){console.error("Erro ao exportar:",e),s.showToast("error","Erro ao exportar relatorio")}}};v.UI=d;const m={verFatura(e,a=null,t=null){const o=new Date;a=a||o.getMonth()+1,t=t||o.getFullYear(),window.location.href=`${f.BASE_URL}faturas?cartao_id=${e}&mes=${a}&ano=${t}`},mostrarModalFatura(e,a=null,t=null,o=null){const r=document.querySelector(".modal-fatura-overlay");r&&r.remove();const n=m.criarModalFatura(e,a,t,o);document.body.appendChild(n),I(),setTimeout(()=>{n.classList.add("show")},10),n.addEventListener("click",c=>{c.target===n&&m.fecharModalFatura(n)}),n.querySelector(".btn-fechar-fatura")?.addEventListener("click",()=>{m.fecharModalFatura(n)}),requestAnimationFrame(()=>{m.setupParcelaSelection(n,e)}),n.querySelector(".btn-pagar-fatura")?.addEventListener("click",()=>{m.pagarParcelasSelecionadas(e)})},setupParcelaSelection(e,a){const t=e.querySelector("#selectAllParcelas"),o=e.querySelectorAll(".parcela-checkbox"),r=e.querySelector("#totalSelecionado");if(e.dataset.parcelasConfigured==="true")return;e.dataset.parcelasConfigured="true";const n=()=>{let c=0;o.forEach(u=>{u.checked&&(c+=parseFloat(u.dataset.valor))}),r&&(r.textContent=s.formatMoney(c))};t&&t.addEventListener("change",c=>{o.forEach(u=>{u.checked=c.target.checked}),n()}),o.forEach(c=>{c.addEventListener("change",()=>{if(n(),t){const u=Array.from(o).every(i=>i.checked);t.checked=u}})}),n()},async pagarParcelasSelecionadas(e){const a=document.querySelectorAll(".parcela-checkbox:checked");if(a.forEach((r,n)=>{}),a.length===0){await Swal.fire({icon:"warning",title:"Atenção",text:"Selecione pelo menos uma parcela para pagar."});return}let t=0;a.forEach(r=>{const n=parseFloat(r.dataset.valor);t+=n}),await s.showConfirmDialog("Confirmar Pagamento",`Deseja pagar ${a.length} parcela(s) no valor total de ${s.formatMoney(t)}?`)&&await v.API.pagarParcelasIndividuais(a,e)},criarModalFatura(e,a=null,t=null,o=null){const r=s.resolverCorCartao(e,o),n=document.createElement("div");return n.className="modal-fatura-overlay",n.innerHTML=`<div class="modal-fatura-container" style="--card-accent: ${r};">${m.criarConteudoModal(e,a,t,o)}</div>`,n},criarConteudoModal(e,a=null,t=null,o=null){const r=o||e.cartao_id||e.cartao?.id;if(t&&t.pago)return m.criarConteudoModalFaturaPaga(e,t,a,r);const n=(e.itens||[]).filter(i=>!i.pago).length,c=(e.itens||[]).filter(i=>i.pago).length,u=e.cartao?.bandeira?s.getBrandIcon(e.cartao.bandeira):null;return`
...
Esta ação criará um lançamento de despesa na conta vinculada e liberará o limite do cartão.`,"Sim, Pagar"))return;const t=document.querySelector(".btn-pagar-fatura"),o=t?t.innerHTML:"";try{t&&(t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="icon-spin"></i> Processando...',I(),t.style.opacity="0.6",t.style.cursor="not-allowed");const r=await G(`${f.API_URL}/cartoes/${e.cartao.id}/fatura/pagar`,{mes:e.mes,ano:e.ano}),n=w(r,null);n?.gamification?.achievements&&Array.isArray(n.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(n.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",`Fatura paga com sucesso! ${n?.itens_pagos??""} parcela(s) quitada(s).`);const c=document.querySelector(".modal-fatura-overlay");c&&m.fecharModalFatura(c),v.API.loadCartoes()}catch(r){console.error("❌ Erro ao pagar fatura:",r),t&&(t.disabled=!1,t.innerHTML=o,t.style.opacity="1",t.style.cursor="pointer"),s.showToast("error",F(r,"Erro ao pagar fatura"))}},criarConteudoModalFaturaPaga(e,a,t,o){const r=o||e.cartao_id||e.cartao?.id,n=(e.itens||[]).filter(i=>i.pago).length,c=e.cartao?.bandeira?s.getBrandIcon(e.cartao.bandeira):null,u=a?.data_pagamento||(e.itens||[]).find(i=>i.pago&&i.data_pagamento)?.data_pagamento||null;return`
        ...
        `},async navegarMes(e,a,t,o){let r=a+o,n=t;r>12?(r=1,n++):r<1&&(r=12,n--);try{const[c,u,i]=await Promise.all([P(`${f.API_URL}/cartoes/${e}/fatura`,{mes:r,ano:n}).catch(()=>null),P(`${f.API_URL}/cartoes/${e}/parcelamentos-resumo`,{mes:r,ano:n}).catch(()=>null),P(`${f.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).catch(()=>null)]);if(!c)throw new Error("Erro ao carregar fatura");const p=c.data||c;let C=null,y=null;u&&(C=u.data||u),i&&(y=i.data||i);const g=document.querySelector(".modal-fatura-container");if(g){const b=s.resolverCorCartao(p,e);g.style.setProperty("--card-accent",b);const h=m.criarConteudoModal(p,C,y,e);g.innerHTML=h,I(),g.querySelector(".btn-fechar-fatura")?.addEventListener("click",()=>{const L=document.querySelector(".modal-fatura-overlay");m.fecharModalFatura(L)}),g.querySelector(".btn-pagar-fatura")?.addEventListener("click",()=>{m.pagarParcelasSelecionadas(p)});const E=document.querySelector(".modal-fatura-overlay");requestAnimationFrame(()=>{m.setupParcelaSelection(E,p)})}}catch(c){console.error("❌ Erro ao navegar entre meses:",c),s.showToast("error",F(c,"Erro ao carregar fatura"))}},async toggleHistoricoFatura(e){try{const a=document.querySelector(".modal-fatura-container");if(!a)return;if(a.querySelector(".historico-faturas")){const o=new Date,r=o.getMonth()+1,n=o.getFullYear(),[c,u,i]=await Promise.all([v.API.carregarFatura(e,r,n),v.API.carregarParcelamentosResumo(e,r,n).catch(()=>null),P(`${f.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).then(C=>w(C,null)).catch(()=>null)]),p=m.criarConteudoModal(c,u,i,e);a.innerHTML=p,I(),m.adicionarEventListenersModal(c)}else{const o=await v.API.carregarHistoricoFaturas(e),r=m.criarConteudoHistorico(o,e);a.innerHTML=r,I(),m.adicionarEventListenersModal(null)}}catch(a){console.error("❌ Erro ao alternar histórico:",a),s.showToast("error","Erro ao carregar histórico")}},criarConteudoHistorico(e,a){return`
            <div class="modal-fatura-header">
