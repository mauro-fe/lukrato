...
`)},setupLimiteMoneyMask(){const e=document.getElementById("limiteTotal");if(!e){console.error("❌ Campo limiteTotal NÃO encontrado!");return}e.addEventListener("input",function(a){let t=a.target.value;t=t.replace(/[^\d]/g,"");const n=((parseInt(t)||0)/100).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".");a.target.value=n}),e.value="0,00"}};function te(e){return typeof e!="string"?e:e.startsWith(y.BASE_URL)?e.slice(y.BASE_URL.length):e}async function $(e,{method:a="GET",data:t=null,headers:o={},timeout:r=15e3}={}){return Y(te(e),{method:a,headers:o,body:t},{timeout:r})}function oe(e){if(l.previewMeta=e?.is_demo?e:null,l.previewMeta){window.LKDemoPreviewBanner?.show(l.previewMeta);return}window.LKDemoPreviewBanner?.hide()}const S={async loadCartoes(){const e=document.getElementById("cartoesGrid"),a=document.getElementById("emptyState"),t=document.getElementById("cartoesContainer");if(!(!e||!a))try{l.isLoading=!0,e.setAttribute("aria-busy","true"),t?.setAttribute("aria-busy","true"),e.innerHTML=`
                <div class="lk-skeleton lk-skeleton--card"></div>
                <div class="lk-skeleton lk-skeleton--card"></div>
                <div class="lk-skeleton lk-skeleton--card"></div>
            `,a.style.display="none";let o;if(window.lkFetch){const n=await window.lkFetch.get(`${y.API_URL}/cartoes?preview=1`,{timeout:2e4,maxRetries:2,showLoading:!0,loadingTarget:"#cartoesContainer"});o=E(n,[])}else o=await $(`${y.API_URL}/cartoes?preview=1`);const r=E(o,{});oe(r?.meta),l.cartoes=Array.isArray(r)?r:Array.isArray(r?.cartoes)?r.cartoes:[],l.previewMeta||await S.verificarFaturasPendentes(),l.lastLoadedAt=new Date().toISOString(),v.UI.updateStats(),v.UI.filterCartoes(),l.previewMeta?(l.alertas=[],S.renderAlertas()):await S.carregarAlertas()}catch(o){B("[Cartoes] Erro ao carregar cartões",o,"Erro ao carregar cartões");let r=F(o,"Erro ao carregar cartoes");o.name==="AbortError"||r.includes("demorou")?r="A conexão está lenta. Tente novamente.":navigator.onLine||(r="Sem conexão com a internet"),s.showToast("error",r),e.innerHTML=`
            ...
            `,I()}finally{l.isLoading=!1,e.setAttribute("aria-busy","false"),t?.setAttribute("aria-busy","false")}},async verificarFaturasPendentes(){l.cartoes.forEach(a=>{a.temFaturaPendente=!1});const e=l.cartoes.map(async a=>{try{const t=await $(`${y.API_URL}/cartoes/${a.id}/faturas-pendentes`),o=E(t,{}),r=o?.meses||E(o,[])||[];a.temFaturaPendente=Array.isArray(r)&&r.length>0}catch{a.temFaturaPendente=!1}});await Promise.all(e)},async carregarAlertas(){try{let e;if(window.lkFetch){const a=await window.lkFetch.get(`${y.API_URL}/cartoes/alertas`,{timeout:1e4,maxRetries:1,showLoading:!1});e=E(a,{});const t=E(e,{});l.alertas=t?.alertas||[]}else{e=await $(`${y.API_URL}/cartoes/alertas`,{timeout:1e4});const a=E(e,{});l.alertas=a?.alertas||[]}S.renderAlertas()}catch(e){J("[Cartoes] Erro ao carregar alertas",e,"Erro ao carregar alertas"),l.alertas=[];const a=document.getElementById("alertasContainer");a&&(a.style.display="none")}},renderAlertas(){const e=document.getElementById("alertasContainer");if(e){if(l.alertas.length===0){e.style.display="none";return}e.style.display="block",e.innerHTML=`
            <div class="alertas-list">
                ${l.alertas.map(a=>S.criarAlertaHTML(a)).join("")}
            </div>
        `,I()}},criarAlertaHTML(e){const a={vencimento_proximo:"calendar-x",limite_baixo:"triangle-alert"},t={critico:"#e74c3c",atencao:"#f39c12"},o=Object.prototype.hasOwnProperty.call(a,e?.tipo)?e.tipo:"limite_baixo",r=Object.prototype.hasOwnProperty.call(t,e?.gravidade)?e.gravidade:"atencao",n=s.escapeHtml(String(e?.nome_cartao||"Cartão")),c=Number(e?.dias_faltando||0),m=Number(e?.percentual_disponivel||0),i=Number(e?.valor_fatura||0),p=Number(e?.limite_disponivel||0);let C="";return o==="vencimento_proximo"?C=`Fatura de <strong>${n}</strong> vence em <strong>${c} dia(s)</strong> - ${s.formatMoney(i)}`:o==="limite_baixo"&&(C=`Limite de <strong>${n}</strong> em <strong>${m.toFixed(1)}%</strong> - ${s.formatMoney(p)} disponível`),`
        ...
        `},dismissAlerta(e){const a=e.closest(".alerta-item");a&&(a.style.animation="slideOut 0.3s ease-out forwards",setTimeout(()=>{a.remove();const t=document.getElementById("alertasContainer");t&&t.querySelectorAll(".alerta-item").length===0&&(t.style.display="none")},300))},async loadContasSelect(){const e=document.getElementById("contaVinculada"),a=document.getElementById("contaVinculadaHelp"),t=document.getElementById("cartaoContaEmptyHint");if(!e){console.error("❌ Select contaVinculada não encontrado!");return}try{const o=`${y.API_URL}/contas?only_active=0&with_balances=1`,r=await $(o),n=E(r,{});let c=[];if(Array.isArray(n)?c=n:Array.isArray(n?.contas)&&(c=n.contas),c.length===0)return e.disabled=!0,e.innerHTML='<option value="">Nenhuma conta disponivel</option>',a&&(a.textContent="Crie uma conta antes de vincular um cartao."),t&&(t.hidden=!1),console.warn("⚠️ Nenhuma conta encontrada"),0;const m=c.map(i=>{const p=i.instituicao_financeira?.nome||i.instituicao?.nome||i.nome||"Sem instituição",C=s.escapeHtml(i.nome||"Conta sem nome"),h=s.escapeHtml(p),b=parseFloat(i.saldoAtual||i.saldo_atual||i.saldo||i.saldo_inicial||0),f=s.formatMoney(b);return`<option value="${i.id}">${C} - ${h} - ${f}</option>`}).join("");return e.disabled=!1,e.innerHTML='<option value="">Selecione a conta</option>'+m,a&&(a.textContent="Conta onde o pagamento da fatura sera debitado."),t&&(t.hidden=!0),c.length}catch(o){return B("[Cartoes] Erro ao carregar contas",o,"Erro ao carregar contas"),e.disabled=!0,e.innerHTML='<option value="">Erro ao carregar contas</option>',a&&(a.textContent="Nao foi possivel carregar as contas agora."),t&&(t.hidden=!1),0}},async saveCartao(){const e=document.getElementById("formCartao");if(!e.checkValidity()){e.reportValidity();return}const a=document.getElementById("cartaoId").value,t=!!a,o=document.querySelector('meta[name="csrf-token"]')?.content||document.querySelector('input[name="csrf_token"]')?.value||"",r=document.getElementById("limiteTotal").value,n=s.parseMoney(r),c=document.getElementById("cartaoLembreteAviso")?.value||"",m=document.getElementById("contaVinculada"),i=document.getElementById("cartaoCanalInapp"),p=document.getElementById("cartaoCanalEmail");if(m?.disabled){s.showToast("error","Crie uma conta antes de cadastrar um cartao.");return}if(c&&!i?.checked&&!p?.checked){s.showToast("error","Selecione pelo menos um canal para o lembrete.");return}const C={nome_cartao:document.getElementById("nomeCartao").value.trim(),conta_id:m?.value?parseInt(m.value,10):null,bandeira:document.getElementById("bandeira").value,ultimos_digitos:document.getElementById("ultimosDigitos").value.trim(),limite_total:n,dia_fechamento:document.getElementById("diaFechamento").value||null,dia_vencimento:document.getElementById("diaVencimento").value||null,lembrar_fatura_antes_segundos:c?parseInt(c):null,fatura_canal_inapp:c&&i?.checked?1:0,fatura_canal_email:c&&p?.checked?1:0,csrf_token:o};try{const h=t?`${y.API_URL}/cartoes/${a}`:`${y.API_URL}/cartoes`,b=await $(h,{method:t?"PUT":"POST",data:C}),f=E(b,null);f?.gamification?.achievements&&Array.isArray(f.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(f.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",t?"Cartão atualizado com sucesso!":"Cartão criado com sucesso!"),v.UI.closeModal(),await S.loadCartoes()}catch(h){B("[Cartoes] Erro ao salvar cartão",h,"Erro ao salvar cartão"),s.showToast("error",F(h,"Erro ao salvar cartao"))}},async editCartao(e){const a=l.cartoes.find(t=>t.id===e);if(a){if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para editar.");return}v.UI.openModal("edit",a)}},async arquivarCartao(e){const a=l.cartoes.find(o=>o.id===e);if(!a)return;if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para arquivar ou editar.");return}if(await s.showConfirmDialog("Arquivar Cartão",`Tem certeza que deseja arquivar o cartão "${a.nome_cartao}"? Você poderá restaurá-lo depois na página de Cartões Arquivados.`,"Arquivar"))try{await $(`${y.API_URL}/cartoes/${e}/archive`,{method:"POST"}),s.showToast("success","Cartão arquivado com sucesso!"),S.loadCartoes()}catch(o){B("[Cartoes] Erro ao arquivar cartão",o,"Erro ao arquivar cartão"),s.showToast("error",F(o,"Erro ao arquivar cartao"))}},async deleteCartao(e){return S.arquivarCartao(e)},async carregarFatura(e,a,t){try{const o=await $(`${y.API_URL}/cartoes/${e}/fatura?mes=${a}&ano=${t}`);return E(o,{itens:[],total:0,pago:0,pendente:0})}catch(o){if(o?.status===404)return{itens:[],total:0,pago:0,pendente:0};throw new Error(F(o,"Erro ao carregar fatura"))}},async carregarParcelamentosResumo(e,a,t){const o=await $(`${y.API_URL}/cartoes/${e}/parcelamentos-resumo?mes=${a}&ano=${t}`);return E(o,null)},async carregarHistoricoFaturas(e,a=12){const t=await $(`${y.API_URL}/cartoes/${e}/faturas-historico?limite=${a}`);return E(t,null)},async pagarParcelasIndividuais(e,a){try{const t=Array.from(e).map(n=>parseInt(n.dataset.id)),o=a.cartao_id||a.cartao?.id;if(!o)throw new Error("ID do cartão não encontrado na fatura");const r=await $(`${y.API_URL}/cartoes/${o}/parcelas/pagar`,{method:"POST",data:{parcela_ids:t,mes:a.mes,ano:a.ano}});if(r?.success!==!1){s.showToast("success",r.message||"Parcelas pagas com sucesso!");const n=document.querySelector(".modal-fatura-overlay");n&&v.Fatura.fecharModalFatura(n),await S.loadCartoes()}else throw new Error(r.message||"Erro ao pagar parcelas")}catch(t){s.showToast("error",F(t,"Erro ao processar a operacao do cartao"))}},async desfazerPagamento(e,a,t){if((await Swal.fire({title:"Desfazer pagamento?",html:`
                <p>Esta ação irá:</p>
            ...
            <article
                ...
                tabindex="0"
                role="button"
                aria-label="Abrir detalhes do cartao ${b}, ${x(r)} do limite usado"
            >
                ...
                <div class="card-header">
                    <div class="card-card-badges">
                        <span class="card-meta-chip card-meta-chip--brand" ${_(`Bandeira ${f}`,"Rede de processamento usada por este cartao para compras e parcelamentos.")}>
                ...
                <div class="card-content">
                    ...
                    <div class="card-subline">
                        <span class="card-last-digits">Final ${M(e.ultimos_digitos,"0000")}</span>
                ...
                <div class="card-details">
                    ...
                    <div class="card-detail-item">
                        ...
                        <strong class="card-detail-value">${w}</strong>
                    </div>
                    <div class="card-detail-item">
                        <span class="card-detail-label">Vencimento</span>
                        <strong class="card-detail-value">${L}</strong>
                    </div>
                </div>
...
Esta ação criará um lançamento de despesa na conta vinculada e liberará o limite do cartão.`,"Sim, Pagar"))return;const t=document.querySelector(".btn-pagar-fatura"),o=t?t.innerHTML:"";try{t&&(t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="icon-spin"></i> Processando...',I(),t.style.opacity="0.6",t.style.cursor="not-allowed");const r=await X(`${y.API_URL}/cartoes/${e.cartao.id}/fatura/pagar`,{mes:e.mes,ano:e.ano}),n=E(r,null);n?.gamification?.achievements&&Array.isArray(n.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(n.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",`Fatura paga com sucesso! ${n?.itens_pagos??""} parcela(s) quitada(s).`);const c=document.querySelector(".modal-fatura-overlay");c&&u.fecharModalFatura(c),v.API.loadCartoes()}catch(r){console.error("❌ Erro ao pagar fatura:",r),t&&(t.disabled=!1,t.innerHTML=o,t.style.opacity="1",t.style.cursor="pointer"),s.showToast("error",F(r,"Erro ao pagar fatura"))}},criarConteudoModalFaturaPaga(e,a,t,o){const r=o||e.cartao_id||e.cartao?.id,n=(e.itens||[]).filter(i=>i.pago).length,c=e.cartao?.bandeira?s.getBrandIcon(e.cartao.bandeira):null,m=a?.data_pagamento||(e.itens||[]).find(i=>i.pago&&i.data_pagamento)?.data_pagamento||null;return`
        ...
        `},async navegarMes(e,a,t,o){let r=a+o,n=t;r>12?(r=1,n++):r<1&&(r=12,n--);try{const[c,m,i]=await Promise.all([T(`${y.API_URL}/cartoes/${e}/fatura`,{mes:r,ano:n}).catch(()=>null),T(`${y.API_URL}/cartoes/${e}/parcelamentos-resumo`,{mes:r,ano:n}).catch(()=>null),T(`${y.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).catch(()=>null)]);if(!c)throw new Error("Erro ao carregar fatura");const p=c.data||c;let C=null,h=null;m&&(C=m.data||m),i&&(h=i.data||i);const b=document.querySelector(".modal-fatura-container");if(b){const f=s.resolverCorCartao(p,e);b.style.setProperty("--card-accent",f);const g=u.criarConteudoModal(p,C,h,e);b.innerHTML=g,I(),b.querySelector(".btn-fechar-fatura")?.addEventListener("click",()=>{const L=document.querySelector(".modal-fatura-overlay");u.fecharModalFatura(L)}),b.querySelector(".btn-pagar-fatura")?.addEventListener("click",()=>{u.pagarParcelasSelecionadas(p)});const w=document.querySelector(".modal-fatura-overlay");requestAnimationFrame(()=>{u.setupParcelaSelection(w,p)})}}catch(c){console.error("❌ Erro ao navegar entre meses:",c),s.showToast("error",F(c,"Erro ao carregar fatura"))}},async toggleHistoricoFatura(e){try{const a=document.querySelector(".modal-fatura-container");if(!a)return;if(a.querySelector(".historico-faturas")){const o=new Date,r=o.getMonth()+1,n=o.getFullYear(),[c,m,i]=await Promise.all([v.API.carregarFatura(e,r,n),v.API.carregarParcelamentosResumo(e,r,n).catch(()=>null),T(`${y.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).then(C=>E(C,null)).catch(()=>null)]),p=u.criarConteudoModal(c,m,i,e);a.innerHTML=p,I(),u.adicionarEventListenersModal(c)}else{const o=await v.API.carregarHistoricoFaturas(e),r=u.criarConteudoHistorico(o,e);a.innerHTML=r,I(),u.adicionarEventListenersModal(null)}}catch(a){console.error("❌ Erro ao alternar histórico:",a),s.showToast("error","Erro ao carregar histórico")}},criarConteudoHistorico(e,a){return`
            <div class="modal-fatura-header">
        ...
        `},adicionarEventListenersModal(e){const a=document.querySelector(".modal-fatura-container");a&&(a.querySelector(".btn-fechar-fatura")?.addEventListener("click",()=>{const t=document.querySelector(".modal-fatura-overlay");u.fecharModalFatura(t)}),e&&a.querySelector(".btn-pagar-fatura")?.addEventListener("click",()=>{u.pagarFatura(e)}))}};v.Fatura=u;const ie={toggleCartoesHero:"cartoesHero",toggleCartoesKpis:"cartoesKpis",toggleCartoesToolbar:"cartoesToolbar"},V={toggleCartoesHero:!0,toggleCartoesKpis:!0,toggleCartoesToolbar:!0},ce={...V,toggleCartoesKpis:!1,toggleCartoesToolbar:!1};async function le(){return ae("cartoes")}async function de(e){await ee("cartoes",e)}const z=Q({storageKey:"lk_cartoes_prefs",sectionMap:ie,completeDefaults:V,essentialDefaults:ce,loadPreferences:le,savePreferences:de,modal:{overlayId:"cartoesCustomizeModalOverlay",openButtonId:"btnCustomizeCartoes",closeButtonId:"btnCloseCustomizeCartoes",saveButtonId:"btnSaveCustomizeCartoes",presetEssentialButtonId:"btnPresetEssencialCartoes",presetCompleteButtonId:"btnPresetCompletoCartoes"}});function j(){z.init()}v.Customize={init:j,open:z.open,close:z.close};const me=async()=>{j(),d.setupEventListeners(),d.restoreViewPreference(),await v.API.loadCartoes()},ue=e=>l.cartoes.find(t=>t.id===e)?.is_demo?(s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para abrir a fatura."),!0):!1;window.cartoesManager={openModal:(e="create",a=null)=>d.openModal(e,a),closeModal:()=>d.closeModal(),moreCartao:(e,a)=>d.showCardMenu(e,a),editCartao:e=>v.API.editCartao(e),arquivarCartao:e=>v.API.arquivarCartao(e),deleteCartao:e=>v.API.deleteCartao(e),exportarRelatorio:()=>d.exportarRelatorio(),mostrarModalFatura:(e,a,t)=>u.mostrarModalFatura(e,a,t),verFatura:e=>{ue(e)||u.verFatura(e)},fecharModalFatura:()=>u.fecharModalFatura(),navegarMes:(e,a,t,o)=>u.navegarMes(e,a,t,o),pagarFatura:(e,a,t)=>u.pagarFatura(e,a,t),pagarParcelasSelecionadas:(e,a)=>u.pagarParcelasSelecionadas(e,a),toggleHistoricoFatura:e=>u.toggleHistoricoFatura(e),dismissAlerta:e=>v.API.dismissAlerta(e),loadCartoes:()=>v.API.loadCartoes(),desfazerPagamento:(e,a,t)=>v.API.desfazerPagamento(e,a,t),desfazerPagamentoParcela:e=>v.API.desfazerPagamentoParcela(e)};window.__CARTOES_MANAGER_INITIALIZED__||(window.__CARTOES_MANAGER_INITIALIZED__=!0,document.addEventListener("DOMContentLoaded",()=>me()));
