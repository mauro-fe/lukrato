import{a as Se}from"./utils-Bj4jxwhy.js";import{c as xe,p as _e,f as Ae}from"./ui-preferences-BzA64XRO.js";import{e as pe,d as R}from"./api-CiEmwEpk.js";const Y="Relatórios são exclusivos do plano Pro.",u={BASE_URL:window.LK?.getBase?.()||"/",CHART_COLORS:["#E67E22","#2C3E50","#2ECC71","#F39C12","#9B59B6","#1ABC9C","#E74C3C","#3498DB"],FETCH_TIMEOUT:3e4,VIEWS:{CATEGORY:"category",BALANCE:"balance",COMPARISON:"comparison",ACCOUNTS:"accounts",CARDS:"cards",EVOLUTION:"evolution",ANNUAL_SUMMARY:"annual_summary",ANNUAL_CATEGORY:"annual_category"}},$e=new Set([u.VIEWS.ANNUAL_SUMMARY,u.VIEWS.ANNUAL_CATEGORY]),z={[u.VIEWS.CATEGORY]:[{value:"despesas_por_categoria",label:"Despesas por categoria"},{value:"receitas_por_categoria",label:"Receitas por categoria"}],[u.VIEWS.ANNUAL_CATEGORY]:[{value:"despesas_anuais_por_categoria",label:"Despesas anuais por categoria"},{value:"receitas_anuais_por_categoria",label:"Receitas anuais por categoria"}]},L={ACTIVE_SECTION:"rel_active_section",ACTIVE_VIEW:"rel_active_view",CATEGORY_TYPE:"rel_category_type",ANNUAL_CATEGORY_TYPE:"rel_annual_category_type"},ne={overview:{kicker:"Painel consolidado",title:"Leia seu mes com contexto",description:"Veja seu pulso financeiro, identifique sinais importantes e acompanhe a evolucao do periodo em um resumo rapido."},relatorios:{kicker:"Relatorio ativo",title:"Transforme lancamentos em decisao",description:"Explore seus numeros por categoria, conta, cartao e evolucao para descobrir onde agir."},insights:{kicker:"Leitura automatica",title:"Insights que ajudam a agir",description:"Receba sinais claros sobre gastos, saldo, concentracoes e oportunidades sem precisar interpretar tudo manualmente."},comparativos:{kicker:"Comparacao temporal",title:"Compare e ajuste sua rota",description:"Entenda o que melhorou, piorou ou estagnou em relacao ao mes e ao ano anteriores."}},ie={[u.VIEWS.CATEGORY]:{title:"Categorias do periodo",description:"Encontre rapidamente onde seu dinheiro esta concentrado por categoria."},[u.VIEWS.BALANCE]:{title:"Saldo diario",description:"Acompanhe como seu caixa evolui ao longo do periodo."},[u.VIEWS.COMPARISON]:{title:"Receitas x despesas",description:"Compare entradas e saidas para entender pressao ou folga no caixa."},[u.VIEWS.ACCOUNTS]:{title:"Desempenho por conta",description:"Descubra quais contas concentram mais entradas e saidas."},[u.VIEWS.CARDS]:{title:"Saude dos cartoes",description:"Monitore faturas, uso de limite e sinais de atencao nos cartoes."},[u.VIEWS.EVOLUTION]:{title:"Evolucao em 12 meses",description:"Observe tendencia, sazonalidade e ritmo financeiro ao longo do ultimo ano."},[u.VIEWS.ANNUAL_SUMMARY]:{title:"Resumo anual",description:"Compare mes a mes como receitas, despesas e saldo se comportaram no ano."},[u.VIEWS.ANNUAL_CATEGORY]:{title:"Categorias do ano",description:"Veja quais categorias dominaram seu ano e onde houve maior concentracao."}};function me(){const e=new Date;return`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,"0")}`}const g=e=>String(e??"").replace(/[&<>"']/g,function(t){return{"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[t]??t});function Te(e,t="#cccccc"){return/^#[0-9A-Fa-f]{6}$/.test(e)?e:t}const i={activeSection:"overview",currentView:u.VIEWS.CATEGORY,categoryType:"despesas_por_categoria",annualCategoryType:"despesas_anuais_por_categoria",currentMonth:me(),currentAccount:null,chart:null,accounts:[],accessRestricted:!1,lastReportError:null,activeDrilldown:null,reportDetails:null},S={getCurrentMonth:me,formatCurrency(e){return Se(e)},formatMonthLabel(e){const[t,a]=e.split("-");return new Date(t,a-1).toLocaleDateString("pt-BR",{month:"long",year:"numeric"})},addMonths(e,t){const[a,s]=e.split("-").map(Number),o=new Date(a,s-1+t);return`${o.getFullYear()}-${String(o.getMonth()+1).padStart(2,"0")}`},hexToRgba(e,t=.25){const a=parseInt(e.slice(1,3),16),s=parseInt(e.slice(3,5),16),o=parseInt(e.slice(5,7),16);return`rgba(${a}, ${s}, ${o}, ${t})`},generateShades(e,t){const a=parseInt(e.slice(1,3),16),s=parseInt(e.slice(3,5),16),o=parseInt(e.slice(5,7),16),r=[];for(let n=0;n<t;n++){const l=.35-n/Math.max(t-1,1)*.7,c=h=>Math.min(255,Math.max(0,Math.round(h+(l>0?(255-h)*l:h*l)))),d=c(a),m=c(s),p=c(o);r.push(`#${d.toString(16).padStart(2,"0")}${m.toString(16).padStart(2,"0")}${p.toString(16).padStart(2,"0")}`)}return r},isYearlyView(e=i.currentView){return $e.has(e)},extractFilename(e){if(!e)return null;const t=/filename\*=UTF-8''([^;]+)/i.exec(e);if(t)try{return decodeURIComponent(t[1])}catch{return t[1]}const a=/filename="?([^";]+)"?/i.exec(e);return a?a[1]:null},getCssVar(e,t=""){try{return(getComputedStyle(document.documentElement).getPropertyValue(e)||"").trim()||t}catch{return t}},isLightTheme(){try{return(document.documentElement?.getAttribute("data-theme")||"dark")==="light"}catch{return!1}},getReportType(){return{[u.VIEWS.CATEGORY]:i.categoryType,[u.VIEWS.ANNUAL_CATEGORY]:i.annualCategoryType,[u.VIEWS.BALANCE]:"saldo_mensal",[u.VIEWS.COMPARISON]:"receitas_despesas_diario",[u.VIEWS.ACCOUNTS]:"receitas_despesas_por_conta",[u.VIEWS.CARDS]:"cartoes_credito",[u.VIEWS.EVOLUTION]:"evolucao_12m",[u.VIEWS.ANNUAL_SUMMARY]:"resumo_anual"}[i.currentView]??i.categoryType},getActiveCategoryType(){return i.currentView===u.VIEWS.ANNUAL_CATEGORY?i.annualCategoryType:i.categoryType}},I={},T=e=>S.formatCurrency(e),F=e=>String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t]||t);function H(){const e=(document.documentElement.getAttribute("data-theme")||"").toLowerCase()==="light"||S.isLightTheme?.();return{isLight:e,mode:e?"light":"dark",textColor:e?"#2c3e50":"#ffffff",textMuted:e?"#6c757d":"rgba(255, 255, 255, 0.7)",gridColor:e?"rgba(0, 0, 0, 0.08)":"rgba(255, 255, 255, 0.05)",surfaceColor:getComputedStyle(document.documentElement).getPropertyValue("--color-surface").trim()}}function Q(e=[]){return e.map(t=>{const a=Number(t);return Number.isFinite(a)?a:0})}function ce(e,t=380){const a=e?.closest(".chart-container")||e?.parentElement,s=a?getComputedStyle(a):null,o=s?Number.parseFloat(s.height):Number.NaN,r=s?Number.parseFloat(s.minHeight):Number.NaN,n=a?.getBoundingClientRect?.().height??Number.NaN,l=s?(Number.parseFloat(s.paddingTop)||0)+(Number.parseFloat(s.paddingBottom)||0):0,c=window.innerWidth<768?320:t,d=[o,n,r,c].find(m=>Number.isFinite(m)&&m>0)??c;return Math.max(260,Math.round(d-l))}const v={_currentEntries:null,destroy(){i.chart&&(Array.isArray(i.chart)?i.chart.forEach(e=>e?.destroy()):i.chart.destroy(),i.chart=null),v._drilldownChart&&(v._drilldownChart.destroy(),v._drilldownChart=null),i.activeDrilldown=null,i.reportDetails=null},setupDefaults(){const e=getComputedStyle(document.documentElement).getPropertyValue("--color-text").trim();window.Apex=window.Apex||{},window.Apex.chart={foreColor:e,fontFamily:"Inter, Arial, sans-serif"},window.Apex.grid={borderColor:"rgba(255, 255, 255, 0.1)"}},renderPie(e){const{labels:t=[],values:a=[],details:s=null,cat_ids:o=null}=e;if(!t.length||!a.some(b=>b>0))return I.UI.showEmptyState();let r=t.map((b,C)=>({label:b,value:Number(a[C])||0,color:u.CHART_COLORS[C%u.CHART_COLORS.length],catId:o?o[C]??null:null})).filter(b=>b.value>0).sort((b,C)=>C.value-b.value);!o&&s&&(r=r.map(b=>{const C=s.find(O=>O.label===b.label);return{...b,catId:C?.cat_id??null}}));const n=window.innerWidth<768;let l=r;if(n&&r.length>5){const b=r.slice(0,5),O=r.slice(5).reduce((V,G)=>V+G.value,0);l=[...b,{label:"Outros",value:O,color:"#95a5a6",isOthers:!0}]}const c=!n&&l.length>2,d=c?Math.ceil(l.length/2):l.length,m=c?[l.slice(0,d),l.slice(d)].filter(b=>b.length):[l],p=`
        ...
        `;I.UI.setContent(p),v.destroy(),i.reportDetails=s,i.activeDrilldown=null,v._currentEntries=l;const h=S.getActiveCategoryType(),x={receitas_por_categoria:"Receitas por Categoria",despesas_por_categoria:"Despesas por Categoria",receitas_anuais_por_categoria:"Receitas anuais por Categoria",despesas_anuais_por_categoria:"Despesas anuais por Categoria"}[h]||"Distribuição por Categoria",$=H();let M=0;i.chart=m.map((b,C)=>{const O=document.getElementById(`chart${C}`);if(!O)return null;const V=b.reduce((E,U)=>E+U.value,0),G=M;M+=b.length;const se=new ApexCharts(O,{chart:{type:"donut",height:"100%",background:"transparent",fontFamily:"Inter, Arial, sans-serif",events:{dataPointSelection:(E,U,oe)=>{const re=G+oe.dataPointIndex,K=l[re];!K||K.isOthers||v.handlePieClick(K,re,oe.dataPointIndex,C)},dataPointMouseEnter:E=>{E.target&&(E.target.style.cursor="pointer")},dataPointMouseLeave:E=>{E.target&&(E.target.style.cursor="default")}}},series:b.map(E=>E.value),labels:b.map(E=>E.label),colors:b.map(E=>E.color),stroke:{width:2,colors:[$.surfaceColor]},plotOptions:{pie:{donut:{size:"60%"},expandOnClick:!0}},legend:{show:!n,position:"bottom",labels:{colors:$.textColor},markers:{shape:"circle"}},title:{text:m.length>1?`${x} - Parte ${C+1}`:x,align:"center",style:{fontSize:"14px",fontWeight:"bold",color:$.textColor}},tooltip:{theme:$.mode,y:{formatter:E=>{const U=V>0?(E/V*100).toFixed(1):"0";return`${T(E)} (${U}%)`}}},dataLabels:{enabled:!1},theme:{mode:$.mode}});return se.render(),se}),n&&v.renderMobileCategoryList(l)},renderMobileCategoryList(e){const t=document.getElementById("categoryListMobile");if(!t)return;const a=e.reduce((r,n)=>r+n.value,0),s=!!i.reportDetails&&window.IS_PRO,o=e.map((r,n)=>{const l=(r.value/a*100).toFixed(1),c=s&&r.catId!=null?i.reportDetails.find(h=>h.cat_id===r.catId):null,d=c&&c.subcategories&&c.subcategories.filter(h=>h.id!==0).length>0,m=d?'<i data-lucide="chevron-down" class="category-chevron"></i>':"";let p="";if(d){const h=S.generateShades(r.color,c.subcategories.length);p=`
                        ...
                        ${c.subcategories.map((y,x)=>{const $=c.total>0?(y.total/c.total*100).toFixed(1):"0.0";return`
                                <div class="drilldown-item drilldown-item-mobile">
                                    ...
                                    <div class="drilldown-info">
                                        <span class="drilldown-name">${F(y.label)}</span>
                                    </div>
    ...
    `}function Ue(e){if(!e)return"";const t=e.atual>=0,a=e.diferenca>0?"trend-positive":e.diferenca<0?"trend-negative":"trend-neutral",s=e.diferenca>0?"arrow-up":e.diferenca<0?"arrow-down":"equal";return`
        <div class="comparative-card comp-mini-card surface-card surface-card--interactive">
    ...
    `}function Fe(e){if(!e||e.length===0)return"";const t={Pix:"zap","Cartão de Crédito":"credit-card","Cartão de Débito":"credit-card",Dinheiro:"banknote",Boleto:"file-text",Depósito:"landmark",Transferência:"arrow-right-left",Estorno:"undo-2"},a=e.reduce((o,r)=>o+r.atual,0);return`
        <div class="comparative-card comp-full-width surface-card surface-card--interactive">
            <div class="comparative-header">
    ...
    `}function He(e){const t=document.getElementById("reportArea");if(!t)return;const a=e.resumo_consolidado&&e.cards&&e.cards.length>0?`
    ...
    `:"";t.innerHTML=`
        <div class="cards-report-container">
            ...
            <div class="cards-grid">
                ${e.cards&&e.cards.length>0?e.cards.map(s=>{const o=Te(s.cor,"#E67E22");return`
    ...
    `,window.lucide&&lucide.createIcons()}const _=e=>S.formatCurrency(e),ge={"arrow-trend-up":"trending-up","arrow-trend-down":"trending-down","arrow-up":"arrow-up","arrow-down":"arrow-down","chart-line":"line-chart","chart-pie":"pie-chart","exclamation-triangle":"triangle-alert","exclamation-circle":"circle-alert","check-circle":"circle-check","info-circle":"info",lightbulb:"lightbulb",star:"star",bolt:"zap",wallet:"wallet","credit-card":"credit-card","calendar-check":"calendar-check",calendar:"calendar",crown:"crown",trophy:"trophy",leaf:"leaf","shield-alt":"shield","money-bill-wave":"banknote","trending-up":"trending-up","trending-down":"trending-down","shield-alert":"shield-alert",gauge:"gauge",target:"target",clock:"clock",receipt:"receipt",calculator:"calculator",layers:"layers","calendar-clock":"calendar-clock","pie-chart":"pie-chart","calendar-range":"calendar-range","list-plus":"list-plus","list-minus":"list-minus","file-text":"file-text","piggy-bank":"piggy-bank",banknote:"banknote"};let q=[];function We(){q.forEach(e=>{try{e.destroy()}catch{}}),q=[]}function Ye(e,t){if(!e)return;const a=t.saldo||0,s=a>=0?"var(--color-success)":"var(--color-danger)",o=a>=0?"positivo":"negativo";let r=`
        <p class="pulse-text">
    ...
    `;t.totalCartoes>0&&(r+=` Faturas de cartões somam <strong>${_(t.totalCartoes)}</strong>.`),r+="</p>",e.innerHTML=r}function ze(e,t){if(e){if(t?.insights?.length>0){e.innerHTML=t.insights.map(a=>{const s=ge[a.icon]||a.icon;return`
                ...
                </div>`}).join("");return}e.innerHTML='<p class="empty-message">Nenhum insight disponível no momento</p>'}}function qe(e,t){if(!e)return;if(!t?.labels?.length){e.innerHTML='<p class="empty-message" style="padding:var(--spacing-6)">Sem dados de categorias</p>';return}e.innerHTML="";const a=5,s=t.labels.slice(0,a),o=t.values.slice(0,a).map(Number);if(t.labels.length>a){const n=t.values.slice(a).reduce((l,c)=>l+Number(c),0);s.push("Outros"),o.push(n)}const r=new ApexCharts(e,{chart:{type:"donut",height:220,background:"transparent"},series:o,labels:s,colors:["#E67E22","#2C3E50","#2ECC71","#F39C12","#9B59B6","#1ABC9C"],legend:{position:"bottom",fontSize:"11px",labels:{colors:"var(--color-text-muted)"}},dataLabels:{enabled:!1},plotOptions:{pie:{donut:{size:"60%"}}},stroke:{show:!1},tooltip:{y:{formatter:n=>_(n)}}});r.render(),q.push(r)}function je(e,t){if(!e)return;if(!t?.labels?.length){e.innerHTML='<p class="empty-message" style="padding:var(--spacing-6)">Sem dados de movimentação</p>';return}e.innerHTML="";const a=(t.receitas||[]).map(Number),s=(t.despesas||[]).map(Number),o=[],r=[],n=[],l=7;for(let d=0;d<t.labels.length;d+=l){const m=Math.floor(d/l)+1;o.push(`Sem ${m}`),r.push(a.slice(d,d+l).reduce((p,h)=>p+h,0)),n.push(s.slice(d,d+l).reduce((p,h)=>p+h,0))}const c=new ApexCharts(e,{chart:{type:"bar",height:220,background:"transparent",toolbar:{show:!1}},series:[{name:"Receitas",data:r},{name:"Despesas",data:n}],colors:["#2ECC71","#E74C3C"],xaxis:{categories:o,labels:{style:{colors:"var(--color-text-muted)",fontSize:"11px"}},axisBorder:{show:!1},axisTicks:{show:!1}},yaxis:{labels:{style:{fontSize:"10px"},formatter:d=>_(d)}},plotOptions:{bar:{columnWidth:"60%",borderRadius:4}},dataLabels:{enabled:!1},legend:{position:"bottom",fontSize:"11px",labels:{colors:"var(--color-text-muted)"}},grid:{borderColor:"rgba(255,255,255,0.05)"},tooltip:{shared:!0,intersect:!1,y:{formatter:d=>_(d)}}});c.render(),q.push(c)}function Ge({API:e}){async function t(){const r=document.getElementById("overviewPulse"),n=document.getElementById("overviewInsights"),l=document.getElementById("overviewCategoryChart"),c=document.getElementById("overviewComparisonChart");We();const[d,m,p,h]=await Promise.all([e.fetchSummaryStats(),e.fetchInsightsTeaser(),e.fetchReportDataForType("despesas_por_categoria",{accountId:null}),e.fetchReportDataForType("receitas_despesas_diario",{accountId:null})]);Ye(r,d),ze(n,m),qe(l,p),je(c,h),window.lucide&&lucide.createIcons()}async function a(){const r=document.getElementById("insightsContainer");if(!r)return;const n=window.IS_PRO?await e.fetchInsights():await e.fetchInsightsTeaser();if(!n||!n.insights||n.insights.length===0){r.innerHTML='<p class="empty-message">Nenhum insight disponível no momento</p>';return}const l=n.insights.map(c=>{const d=ge[c.icon]||c.icon;return`
            ...
            `);try{const c=r.type,d=r.format,m=new URLSearchParams({type:c,format:d,year:i.currentMonth.split("-")[0],month:i.currentMonth.split("-")[1]});i.currentAccount&&m.set("account_id",i.currentAccount);const p=await fetch(`${u.BASE_URL}api/reports/export?${m}`,{credentials:"include"});if(await a(p))return;if(!p.ok){let b="Erro ao exportar relatório.";try{const C=await p.json();C?.message?b=C.message:C?.errors&&(b=Object.values(C.errors).flat().join(", "))}catch{}throw new Error(b)}const h=await p.blob(),y=p.headers.get("Content-Disposition"),x=S.extractFilename(y)||(d==="excel"?"relatorio.xlsx":"relatorio.pdf"),$=URL.createObjectURL(h),M=document.createElement("a");M.href=$,M.download=x,document.body.appendChild(M),M.click(),M.remove(),URL.revokeObjectURL($),typeof Swal<"u"&&Swal.fire({toast:!0,position:"top-end",icon:"success",title:"Relatório exportado!",text:x,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}catch(c){console.error("Export error:",c);const d=pe(c,"Erro ao exportar relatório. Tente novamente.");typeof Swal<"u"?Swal.fire({toast:!0,position:"top-end",icon:"error",title:"Erro ao exportar",text:d,showConfirmButton:!1,timer:3e3}):alert(d)}finally{n&&(n.disabled=!1,n.innerHTML=l)}}}const ve=e=>S.formatMonthLabel(e),D=e=>S.isYearlyView(e),Qe=()=>S.getReportType(),fe=()=>S.getActiveCategoryType();function Z(e=i.currentAccount){return e?i.accounts.find(t=>String(t.id)===String(e))?.name||`Conta #${e}`:null}function J(){return D()?`Ano ${i.currentMonth.split("-")[0]}`:ve(i.currentMonth)}function de(){const e=fe();return(z[i.currentView]||[]).find(a=>a.value===e)?.label||null}function Ze(e=i.activeSection){return e==="relatorios"||e==="comparativos"}function Je(e=i.activeSection){return ne[e]||ne.overview}function X(e=i.currentView){return ie[e]||ie[u.VIEWS.CATEGORY]}function be(){try{localStorage.setItem(L.ACTIVE_VIEW,i.currentView),localStorage.setItem(L.CATEGORY_TYPE,i.categoryType),localStorage.setItem(L.ANNUAL_CATEGORY_TYPE,i.annualCategoryType)}catch{}}function te(){typeof window.openBillingModal=="function"?window.openBillingModal():location.href=`${u.BASE_URL}billing`}async function we(e){const t=e||Y;window.PlanLimits?.promptUpgrade?await window.PlanLimits.promptUpgrade({context:"relatorios",message:t}):window.LKFeedback?.upgradePrompt?await window.LKFeedback.upgradePrompt({context:"relatorios",message:t}):window.Swal?.fire?(await Swal.fire({title:"Recurso exclusivo",text:t,icon:"info",showCancelButton:!0,confirmButtonText:"Assinar plano Pro",cancelButtonText:"Agora não",reverseButtons:!0,focusConfirm:!0})).isConfirmed&&te():confirm(`${t}

Deseja ir para a página de planos agora?`)&&te()}async function k(e){if(!e)return!1;const t=Number(e.status||e?.data?.status||0);if(t===401){const a=encodeURIComponent(location.pathname+location.search);return location.href=`${u.BASE_URL}login?return=${a}`,!0}if(t===403){let a=Y;if(e?.data?.message)a=e.data.message;else if(typeof e?.clone=="function")try{const s=await e.clone().json();s?.message&&(a=s.message)}catch{}return i.accessRestricted||(i.accessRestricted=!0,await we(a)),w.showPaywall(a),!0}return!1}function Xe(e){typeof Swal<"u"&&Swal.fire({toast:!0,position:"top-end",icon:"error",title:e,showConfirmButton:!1,timer:4e3,timerProgressBar:!0})}const j={async fetchReportData(){i.lastReportError=null;const e=new AbortController,t=setTimeout(()=>e.abort(),u.FETCH_TIMEOUT);try{const a=await R(`${u.BASE_URL}api/reports`,{type:S.getReportType(),year:i.currentMonth.split("-")[0],month:i.currentMonth.split("-")[1],account_id:i.currentAccount||void 0});return clearTimeout(t),i.accessRestricted=!1,i.lastReportError=null,a.data||a}catch(a){return clearTimeout(t),await k(a)||(i.lastReportError=a.name==="AbortError"?"A requisição demorou demais. Tente novamente em instantes.":"Não foi possível carregar o relatório agora. Verifique a conexão e tente novamente.",console.error("Error fetching report data:",a),Xe(pe(a,"Erro ao carregar relatório. Verifique sua conexão."))),null}},async fetchReportDataForType(e,t={}){const a=new URLSearchParams({type:e,year:i.currentMonth.split("-")[0],month:i.currentMonth.split("-")[1]}),s=Object.prototype.hasOwnProperty.call(t,"accountId")?t.accountId:i.currentAccount;s&&a.set("account_id",s);try{const o=await R(`${u.BASE_URL}api/reports`,Object.fromEntries(a.entries()));return o.data||o}catch{return null}},async fetchAccounts(){try{const e=await R(`${u.BASE_URL}api/contas`);i.accessRestricted=!1;const t=e.data||e.items||e||[];return(Array.isArray(t)?t:[]).map(a=>({id:Number(a.id),name:a.nome||a.apelido||a.instituicao||`Conta #${a.id}`}))}catch(e){return await k(e)?[]:(console.error("Error fetching accounts:",e),[])}},async fetchSummaryStats(){const[e,t]=i.currentMonth.split("-"),a=new AbortController,s=setTimeout(()=>a.abort(),u.FETCH_TIMEOUT);try{const o=await R(`${u.BASE_URL}api/reports/summary`,{year:e,month:t});return clearTimeout(s),o.data||o}catch(o){return clearTimeout(s),await k(o)?{totalReceitas:0,totalDespesas:0,saldo:0,totalCartoes:0}:(console.error("Error fetching summary stats:",o),{totalReceitas:0,totalDespesas:0,saldo:0,totalCartoes:0})}},async fetchInsights(){const[e,t]=i.currentMonth.split("-"),a=new AbortController,s=setTimeout(()=>a.abort(),u.FETCH_TIMEOUT);try{const o=await R(`${u.BASE_URL}api/reports/insights`,{year:e,month:t});return clearTimeout(s),o.data||o}catch(o){return clearTimeout(s),await k(o)?{insights:[]}:(console.error("Error fetching insights:",o),{insights:[]})}},async fetchInsightsTeaser(){const[e,t]=i.currentMonth.split("-"),a=new AbortController,s=setTimeout(()=>a.abort(),u.FETCH_TIMEOUT);try{const o=await R(`${u.BASE_URL}api/reports/insights-teaser`,{year:e,month:t});return clearTimeout(s),o.data||o}catch(o){return clearTimeout(s),console.error("Error fetching insights teaser:",o),{insights:[],totalCount:0,isTeaser:!0}}},async fetchComparatives(){const[e,t]=i.currentMonth.split("-"),a=new URLSearchParams({year:e,month:t});i.currentAccount&&a.set("account_id",i.currentAccount);const s=new AbortController,o=setTimeout(()=>s.abort(),u.FETCH_TIMEOUT);try{const r=await R(`${u.BASE_URL}api/reports/comparatives`,Object.fromEntries(a.entries()));return clearTimeout(o),r.data||r}catch(r){return clearTimeout(o),await k(r)||console.error("Error fetching comparatives:",r),null}}};I.API=j;const w={setContent(e){const t=document.getElementById("reportArea");t&&(t.innerHTML=e,t.setAttribute("aria-busy","false"),window.lucide&&lucide.createIcons())},showLoading(){const e=document.getElementById("reportArea");e&&(e.setAttribute("aria-busy","true"),e.innerHTML=`
        ...
        `:"",window.lucide&&lucide.createIcons()},updateReportFilterSummary(){const e=document.getElementById("reportFilterSummary"),t=document.getElementById("reportScopeNote");if(!e||!t)return;const a=[`<span class="report-filter-chip surface-chip"><i data-lucide="calendar-range"></i><span>${g(J())}</span></span>`,`<span class="report-filter-chip surface-chip"><i data-lucide="bar-chart-3"></i><span>${g(X().title)}</span></span>`],s=de();s&&a.push(`<span class="report-filter-chip surface-chip"><i data-lucide="filter"></i><span>${g(s)}</span></span>`),i.currentAccount?a.push(`<span class="report-filter-chip surface-chip surface-chip--highlight report-filter-chip-highlight"><i data-lucide="landmark"></i><span>${g(Z())}</span></span>`):a.push('<span class="report-filter-chip surface-chip"><i data-lucide="layers"></i><span>Todas as contas</span></span>'),e.innerHTML=a.join(""),t.classList.remove("hidden"),t.innerHTML=i.currentAccount?'<i data-lucide="info"></i><span>O resumo do topo continua consolidado. O filtro por conta afeta este gráfico e a aba Comparativos.</span>':'<i data-lucide="info"></i><span>Use o filtro de conta para analisar um recorte específico sem perder o consolidado do topo.</span>',window.lucide&&lucide.createIcons()},updateControls(){const e=document.getElementById("typeSelectWrapper"),t=[u.VIEWS.CATEGORY,u.VIEWS.ANNUAL_CATEGORY].includes(i.currentView);e&&(e.classList.toggle("hidden",!t),t&&w.syncTypeSelect());const a=document.getElementById("accountSelectWrapper");a&&a.classList.remove("hidden")},syncTypeSelect(){const e=document.getElementById("reportType");if(!e)return;const t=z[i.currentView];if(!t)return;(e.options.length!==t.length||t.some((s,o)=>e.options[o]?.value!==s.value))&&(e.innerHTML=t.map(s=>`<option value="${s.value}">${s.label}</option>`).join("")),e.value=fe()},setActiveTab(e){document.querySelectorAll(".tab-btn").forEach(t=>{const a=t.dataset.view===e;t.classList.toggle("active",a),t.setAttribute("aria-selected",a)})}};I.UI=w;const et=()=>j.fetchReportData(),tt=()=>w.showLoading(),ee=()=>w.showEmptyState(),at=e=>w.showErrorState(e),st=()=>w.updateMonthLabel(),B=()=>w.updatePageContext(),P=()=>w.updateReportFilterSummary(),ot=()=>w.updateControls(),rt=e=>w.setActiveTab(e),nt=e=>v.renderPie(e),it=e=>v.renderLine(e),ct=e=>v.renderBar(e);async function A(){B(),P(),tt(),lt();const e=await et();if(!i.accessRestricted){if(i.lastReportError)return at(i.lastReportError);if(i.currentView===u.VIEWS.CARDS){if(!e||!Array.isArray(e.cards))return ee();He(e);return}if(!e||!e.labels||e.labels.length===0)return ee();switch(i.currentView){case u.VIEWS.CATEGORY:case u.VIEWS.ANNUAL_CATEGORY:nt(e);break;case u.VIEWS.BALANCE:case u.VIEWS.EVOLUTION:it(e);break;case u.VIEWS.COMPARISON:case u.VIEWS.ACCOUNTS:case u.VIEWS.ANNUAL_SUMMARY:ct(e);break;default:ee()}Be(e)}}const{updateSummaryCards:lt,updateInsightsSection:dt,updateOverviewSection:ut,updateComparativesSection:pt}=Ge({API:j}),mt=Ke({getReportType:Qe,showRestrictionAlert:we,handleRestrictedAccess:k});async function ht(e){e==="overview"?await ut():e==="relatorios"?await A():e==="insights"?await dt():e==="comparativos"&&await pt()}function ye(){const e=D();if(window.LukratoHeader?.setPickerMode?.(e?"year":"month"),e){const t=window.LukratoHeader?.getYear?.();if(t){const[,a="01"]=i.currentMonth.split("-"),s=String(a).padStart(2,"0");i.currentMonth=`${t}-${s}`}}}function Ce(e){i.currentView=e,rt(e),ot(),B(),P(),ye(),be(),A()}function Ee(e){i.currentView===u.VIEWS.ANNUAL_CATEGORY?i.annualCategoryType=e:i.categoryType=e,B(),P(),be(),A()}function ae(e){i.currentAccount=e||null,B(),P(),A()}function gt(e){!e?.detail?.month||D()||i.currentMonth!==e.detail.month&&(i.currentMonth=e.detail.month,st(),B(),P(),A())}function vt(e){if(!D()||!e?.detail?.year)return;const[,t="01"]=i.currentMonth.split("-"),a=String(t).padStart(2,"0"),s=`${e.detail.year}-${a}`;i.currentMonth!==s&&(i.currentMonth=s,B(),P(),A())}function ft(){try{const e=localStorage.getItem(L.ACTIVE_VIEW);e&&Object.values(u.VIEWS).includes(e)&&(i.currentView=e);const t=localStorage.getItem(L.CATEGORY_TYPE);t&&z[u.VIEWS.CATEGORY]?.some(s=>s.value===t)&&(i.categoryType=t);const a=localStorage.getItem(L.ANNUAL_CATEGORY_TYPE);a&&z[u.VIEWS.ANNUAL_CATEGORY]?.some(s=>s.value===a)&&(i.annualCategoryType=a)}catch{}}function bt(){const e=s=>{i.activeSection=s,document.querySelectorAll(".rel-section-tab").forEach(n=>{n.classList.remove("active"),n.setAttribute("aria-selected","false")}),document.querySelectorAll(".rel-section-panel").forEach(n=>{n.classList.remove("active")});const o=document.querySelector(`.rel-section-tab[data-section="${s}"]`);o&&(o.classList.add("active"),o.setAttribute("aria-selected","true"));const r=document.getElementById(`section-${s}`);r&&r.classList.add("active"),localStorage.setItem(L.ACTIVE_SECTION,s),w.updatePageContext(),ht(s),window.lucide?.createIcons?.()},t=["comparativos"];document.querySelectorAll(".rel-section-tab").forEach(s=>{s.addEventListener("click",()=>{const o=s.dataset.section;if(!window.IS_PRO&&t.includes(o)){window.PlanLimits?.promptUpgrade?window.PlanLimits.promptUpgrade({context:"relatorios",message:"Esta funcionalidade e exclusiva do plano Pro."}).catch(()=>{}):window.LKFeedback?.upgradePrompt?window.LKFeedback.upgradePrompt({context:"relatorios",message:"Esta funcionalidade e exclusiva do plano Pro."}).catch(()=>{}):Swal.fire({icon:"info",title:"Recurso Premium",html:"Esta funcionalidade e exclusiva do <b>plano Pro</b>.<br>Faca upgrade para desbloquear!",confirmButtonText:'<i class="lucide-crown" style="margin-right:6px"></i> Fazer Upgrade',showCancelButton:!0,cancelButtonText:"Agora nao",confirmButtonColor:"#f59e0b",cancelButtonColor:"#64748b"}).then(r=>{r.isConfirmed&&(window.location.href=`${window.BASE_URL||"/"}billing`)});return}e(o)})});const a=localStorage.getItem(L.ACTIVE_SECTION);if(a&&document.getElementById(`section-${a}`)){!window.IS_PRO&&t.includes(a)?e("overview"):e(a);return}e("overview")}function wt(e,t){const a=document.getElementById("clearFiltersWrapper"),s=document.getElementById("btnLimparFiltrosRel"),o=()=>{if(!a)return;const r=e&&e.selectedIndex>0,n=t&&t.value!=="";a.style.display=r||n?"flex":"none"};return e?.addEventListener("change",o),t?.addEventListener("change",o),s?.addEventListener("click",()=>{e&&(e.selectedIndex=0,Ee(e.value)),t&&(t.value="",ae("")),o()}),o(),o}function yt(e,t){document.addEventListener("click",a=>{if(a.target.closest('[data-action="retry-report"]')){a.preventDefault(),A();return}if(a.target.closest('[data-action="clear-report-account"]')){a.preventDefault(),e&&(e.value=""),ae(""),typeof t=="function"&&t();return}const r=a.target.closest('[data-action="open-card-detail"]');if(!r)return;a.stopPropagation();const n=parseInt(r.dataset.cardId,10),l=r.dataset.cardNome||"",c=r.dataset.cardCor||"#E67E22",d=r.dataset.cardMonth||i.currentMonth;if(n){if(window.LK_CardDetail?.open){window.LK_CardDetail.open(n,l,c,d);return}console.error("[Relatorios] LK_CardDetail module not loaded"),typeof Swal<"u"&&Swal.fire({toast:!0,position:"top-end",icon:"error",title:"Modulo de detalhes nao carregado",text:"Recarregue a pagina.",showConfirmButton:!1,timer:3e3})}})}function Ct(){window.ReportsAPI={setMonth:e=>{/^\d{4}-\d{2}$/.test(e)&&(i.currentMonth=e,w.updateMonthLabel(),A())},setView:e=>{Object.values(u.VIEWS).includes(e)&&Ce(e)},refresh:()=>A(),getState:()=>({...i})}}async function ue(){ke(),v.setupDefaults(),i.accounts=await j.fetchAccounts();const e=document.getElementById("accountFilter");e&&i.accounts.forEach(o=>{const r=document.createElement("option");r.value=o.id,r.textContent=o.name,e.appendChild(r)}),ft(),document.querySelectorAll(".tab-btn").forEach(o=>{o.addEventListener("click",()=>Ce(o.dataset.view))}),w.setActiveTab(i.currentView),w.updateControls(),w.updatePageContext(),bt();const t=document.getElementById("reportType");t?.addEventListener("change",o=>Ee(o.target.value)),e?.addEventListener("change",o=>ae(o.target.value));const a=wt(t,e);document.addEventListener("lukrato:theme-changed",()=>{v.setupDefaults(),A()});const s=window.LukratoHeader?.getMonth?.();s&&(i.currentMonth=s),document.addEventListener("lukrato:month-changed",gt),document.addEventListener("lukrato:year-changed",vt),document.getElementById("exportBtn")?.addEventListener("click",mt),yt(e,a),ye(),w.updateMonthLabel(),w.updateControls(),await A()}function Et(){window.__LK_REPORTS_LOADED__||(window.__LK_REPORTS_LOADED__=!0,document.readyState==="loading"?document.addEventListener("DOMContentLoaded",()=>{ue()}):ue(),Ct())}Et();
