        ...
        `;I.UI.setContent(m),w.destroy(),n.reportDetails=s,n.activeDrilldown=null,w._currentEntries=i;const d=x.getActiveCategoryType(),E={receitas_por_categoria:"Receitas por Categoria",despesas_por_categoria:"Despesas por Categoria",receitas_anuais_por_categoria:"Receitas anuais por Categoria",despesas_anuais_por_categoria:"Despesas anuais por Categoria"}[d]||"Distribuição por Categoria",f=H();let R=0;n.chart=h.map((y,C)=>{const M=document.getElementById(`chart${C}`);if(!M)return null;const P=y.reduce((A,U)=>A+U.value,0),j=R;R+=y.length;const ae=new ApexCharts(M,{chart:{type:"donut",height:"100%",background:"transparent",fontFamily:"Inter, Arial, sans-serif",events:{dataPointSelection:(A,U,se)=>{const re=j+se.dataPointIndex,G=i[re];!G||G.isOthers||w.handlePieClick(G,re,se.dataPointIndex,C)},dataPointMouseEnter:A=>{A.target&&(A.target.style.cursor="pointer")},dataPointMouseLeave:A=>{A.target&&(A.target.style.cursor="default")}}},series:y.map(A=>A.value),labels:y.map(A=>A.label),colors:y.map(A=>A.color),stroke:{width:2,colors:[f.surfaceColor]},plotOptions:{pie:{donut:{size:"60%"},expandOnClick:!0}},legend:{show:!l,position:"bottom",labels:{colors:f.textColor},markers:{shape:"circle"}},title:{text:h.length>1?`${E} - Parte ${C+1}`:E,align:"center",style:{fontSize:"14px",fontWeight:"bold",color:f.textColor}},tooltip:{theme:f.mode,y:{formatter:A=>{const U=P>0?(A/P*100).toFixed(1):"0";return`${T(A)} (${U}%)`}}},dataLabels:{enabled:!1},theme:{mode:f.mode}});return ae.render(),ae}),l&&w.renderMobileCategoryList(i)},renderMobileCategoryList(e){const a=document.getElementById("categoryListMobile");if(!a)return;const t=e.reduce((o,l)=>o+l.value,0),s=!!n.reportDetails&&window.IS_PRO,r=e.map((o,l)=>{const i=(o.value/t*100).toFixed(1),c=s&&o.catId!=null?n.reportDetails.find(d=>d.cat_id===o.catId):null,p=c&&c.subcategories&&c.subcategories.filter(d=>d.id!==0).length>0,h=p?'<i data-lucide="chevron-down" class="category-chevron"></i>':"";let m="";if(p){const d=x.generateShades(o.color,c.subcategories.length);m=`
                ...
                <div class="category-item ${p?"has-subcats":""}"
                    ...
                    <div class="category-info">
                        ...
                        <span class="category-value">${T(o.value)}</span>
                    </div>
        ...
        `;t.innerHTML=`
            <div class="drilldown-header" style="border-left-color: ${a}">
                <div class="drilldown-title">
                    ...
                    <h4>${F(s)}</h4>
                    <span class="drilldown-total">${T(r)}</span>
                </div>
...
Deseja ir para a página de planos agora?`)&&te()}async function B(e){if(!e)return!1;const a=Number(e.status||e?.data?.status||0);if(a===401){const t=encodeURIComponent(location.pathname+location.search);return location.href=`${u.BASE_URL}login?return=${t}`,!0}if(a===403){let t=q;if(e?.data?.message)t=e.data.message;else if(typeof e?.clone=="function")try{const s=await e.clone().json();s?.message&&(t=s.message)}catch{}return n.accessRestricted||(n.accessRestricted=!0,await fe(t)),S.showPaywall(t),!0}return!1}function Te(e){typeof Swal<"u"&&Swal.fire({toast:!0,position:"top-end",icon:"error",title:e,showConfirmButton:!1,timer:4e3,timerProgressBar:!0})}const $={async fetchReportData(){n.lastReportError=null;const e=new AbortController,a=setTimeout(()=>e.abort(),u.FETCH_TIMEOUT);try{const t=await k(`${u.BASE_URL}api/reports`,{type:x.getReportType(),year:n.currentMonth.split("-")[0],month:n.currentMonth.split("-")[1],account_id:n.currentAccount||void 0});return clearTimeout(a),n.accessRestricted=!1,n.lastReportError=null,t.data||t}catch(t){return clearTimeout(a),await B(t)||(n.lastReportError=t.name==="AbortError"?"A requisição demorou demais. Tente novamente em instantes.":"Não foi possível carregar o relatório agora. Verifique a conexão e tente novamente.",console.error("Error fetching report data:",t),Te(pe(t,"Erro ao carregar relatório. Verifique sua conexão."))),null}},async fetchReportDataForType(e,a={}){const t=new URLSearchParams({type:e,year:n.currentMonth.split("-")[0],month:n.currentMonth.split("-")[1]}),s=Object.prototype.hasOwnProperty.call(a,"accountId")?a.accountId:n.currentAccount;s&&t.set("account_id",s);try{const r=await k(`${u.BASE_URL}api/reports`,Object.fromEntries(t.entries()));return r.data||r}catch{return null}},async fetchAccounts(){try{const e=await k(`${u.BASE_URL}api/contas`);n.accessRestricted=!1;const a=e.data||e.items||e||[];return(Array.isArray(a)?a:[]).map(t=>({id:Number(t.id),name:t.nome||t.apelido||t.instituicao||`Conta #${t.id}`}))}catch(e){return await B(e)?[]:(console.error("Error fetching accounts:",e),[])}},async fetchSummaryStats(){const[e,a]=n.currentMonth.split("-"),t=new AbortController,s=setTimeout(()=>t.abort(),u.FETCH_TIMEOUT);try{const r=await k(`${u.BASE_URL}api/reports/summary`,{year:e,month:a});return clearTimeout(s),r.data||r}catch(r){return clearTimeout(s),await B(r)?{totalReceitas:0,totalDespesas:0,saldo:0,totalCartoes:0}:(console.error("Error fetching summary stats:",r),{totalReceitas:0,totalDespesas:0,saldo:0,totalCartoes:0})}},async fetchInsights(){const[e,a]=n.currentMonth.split("-"),t=new AbortController,s=setTimeout(()=>t.abort(),u.FETCH_TIMEOUT);try{const r=await k(`${u.BASE_URL}api/reports/insights`,{year:e,month:a});return clearTimeout(s),r.data||r}catch(r){return clearTimeout(s),await B(r)?{insights:[]}:(console.error("Error fetching insights:",r),{insights:[]})}},async fetchInsightsTeaser(){const[e,a]=n.currentMonth.split("-"),t=new AbortController,s=setTimeout(()=>t.abort(),u.FETCH_TIMEOUT);try{const r=await k(`${u.BASE_URL}api/reports/insights-teaser`,{year:e,month:a});return clearTimeout(s),r.data||r}catch(r){return clearTimeout(s),console.error("Error fetching insights teaser:",r),{insights:[],totalCount:0,isTeaser:!0}}},async fetchComparatives(){const[e,a]=n.currentMonth.split("-"),t=new URLSearchParams({year:e,month:a});n.currentAccount&&t.set("account_id",n.currentAccount);const s=new AbortController,r=setTimeout(()=>s.abort(),u.FETCH_TIMEOUT);try{const o=await k(`${u.BASE_URL}api/reports/comparatives`,Object.fromEntries(t.entries()));return clearTimeout(r),o.data||o}catch(o){return clearTimeout(r),await B(o)||console.error("Error fetching comparatives:",o),null}}};I.API=$;const S={setContent(e){const a=document.getElementById("reportArea");a&&(a.innerHTML=e,a.setAttribute("aria-busy","false"),window.lucide&&lucide.createIcons())},showLoading(){const e=document.getElementById("reportArea");e&&(e.setAttribute("aria-busy","true"),e.innerHTML=`
    ...
    `}).join("");if(e.innerHTML=s,!window.IS_PRO&&a.isTeaser){const r=Math.max(0,(a.totalCount||0)-a.insights.length),o=r>0?`Desbloqueie mais ${r} insights com PRO`:"Desbloqueie todos os insights com PRO";e.insertAdjacentHTML("beforeend",`
        ...
        `)}window.lucide&&lucide.createIcons()}let Y=[];async function ye(){const e=document.getElementById("overviewPulse"),a=document.getElementById("overviewInsights"),t=document.getElementById("overviewCategoryChart"),s=document.getElementById("overviewComparisonChart");Y.forEach(c=>{try{c.destroy()}catch{}}),Y=[];const[r,o,l,i]=await Promise.all([$.fetchSummaryStats(),$.fetchInsightsTeaser(),$.fetchReportDataForType("despesas_por_categoria",{accountId:null}),$.fetchReportDataForType("receitas_despesas_diario",{accountId:null})]);if(e){const c=r.saldo||0,p=c>=0?"var(--color-success)":"var(--color-danger)",h=c>=0?"positivo":"negativo";let m=`
            <p class="pulse-text">
                Neste mês você recebeu <strong>${v(r.totalReceitas)}</strong>
                e gastou <strong>${v(r.totalDespesas)}</strong>.
                Seu saldo é <strong style="color:${p}">${h} em ${v(Math.abs(c))}</strong>.
        `;r.totalCartoes>0&&(m+=` Faturas de cartões somam <strong>${v(r.totalCartoes)}</strong>.`),m+="</p>",e.innerHTML=m}if(a)if(o?.insights?.length>0){const c={"arrow-trend-up":"trending-up","arrow-trend-down":"trending-down","exclamation-triangle":"triangle-alert","check-circle":"circle-check","shield-alert":"shield-alert",gauge:"gauge",target:"target",receipt:"receipt",calculator:"calculator",layers:"layers","piggy-bank":"piggy-bank","pie-chart":"pie-chart","calendar-range":"calendar-range","calendar-clock":"calendar-clock","credit-card":"credit-card","trending-up":"trending-up","trending-down":"trending-down","list-plus":"list-plus","list-minus":"list-minus",banknote:"banknote",clock:"clock"};a.innerHTML=o.insights.map(p=>{const h=c[p.icon]||p.icon;return`
    ...
    `}let D=null;function He(e){if(!e||e.length===0)return;const a=document.getElementById("evolucaoMiniChart");if(!a)return;const t=e.map(i=>i.label),r=getComputedStyle(document.documentElement).getPropertyValue("--color-text-muted").trim()||"#999",l=document.documentElement.getAttribute("data-theme")==="dark"?"dark":"light";D&&(D.destroy(),D=null),D=new ApexCharts(a,{chart:{type:"line",height:260,stacked:!1,toolbar:{show:!1},background:"transparent",fontFamily:"Inter, Arial, sans-serif"},series:[{name:"Receitas",type:"column",data:e.map(i=>i.receitas)},{name:"Despesas",type:"column",data:e.map(i=>i.despesas)},{name:"Saldo",type:"area",data:e.map(i=>i.saldo)}],xaxis:{categories:t,labels:{style:{colors:r}},axisBorder:{show:!1},axisTicks:{show:!1}},yaxis:{labels:{style:{colors:r},formatter:i=>v(i)}},colors:["rgba(46, 204, 113, 0.85)","rgba(231, 76, 60, 0.85)","#3498db"],stroke:{width:[0,0,2.5],curve:"smooth"},fill:{opacity:[.85,.85,.1]},plotOptions:{bar:{borderRadius:6,columnWidth:"55%"}},grid:{borderColor:"rgba(128,128,128,0.1)",strokeDashArray:4,xaxis:{lines:{show:!1}}},tooltip:{theme:l,shared:!0,intersect:!1,y:{formatter:i=>v(i)}},legend:{position:"bottom",labels:{colors:r},markers:{shape:"circle"}},dataLabels:{enabled:!1},theme:{mode:l}}),D.render()}function We(e){if(!e)return"";const a=e.variacao>0?"trend-negative":e.variacao<0?"trend-positive":"trend-neutral",t=e.variacao>0?"arrow-up":e.variacao<0?"arrow-down":"equal";return`
    ...
    `}function qe(e){if(!e||e.length===0)return"";const a={Pix:"zap","Cartão de Crédito":"credit-card","Cartão de Débito":"credit-card",Dinheiro:"banknote",Boleto:"file-text",Depósito:"landmark",Transferência:"arrow-right-left",Estorno:"undo-2"},t=e.reduce((r,o)=>r+o.atual,0);return`
        <div class="comparative-card comp-full-width surface-card surface-card--interactive">
            <div class="comparative-header">
            ...
            <div class="forma-comp-list">
                ${e.map((r,o)=>{const l=t>0?(r.atual/t*100).toFixed(0):0,i=a[r.nome]||"wallet";return`
            <div class="forma-comp-row" style="animation-delay: ${o*.06}s">
                ...
                <div class="forma-comp-info">
                    <span class="forma-comp-name">${b(r.nome)}</span>
                    <div class="forma-comp-bar-bg">
                        <div class="forma-comp-bar" style="width: ${l}%"></div>
                    </div>
                </div>
                <div class="forma-comp-values">
                    <span class="forma-comp-current">${v(r.atual)} <small>(${r.atual_qtd}x)</small></span>
                    <span class="forma-comp-prev">${v(r.anterior)} <small>(${r.anterior_qtd}x)</small></span>
                </div>
            </div>
        `}).join("")}
            </div>
        </div>
    `}function le(e,a,t){const s=(c,p=!1)=>c>0?'<i data-lucide="arrow-up"></i>':c<0?'<i data-lucide="arrow-down"></i>':'<i data-lucide="equal"></i>',r=(c,p=!1)=>{if(p){if(c>0)return"trend-negative";if(c<0)return"trend-positive"}else{if(c>0)return"trend-positive";if(c<0)return"trend-negative"}return"trend-neutral"},o=(c,p=!1)=>Math.abs(c)<.1?"Sem alteração":c>0?`Aumentou ${Math.abs(c).toFixed(1)}%`:c<0?`Reduziu ${Math.abs(c).toFixed(1)}%`:"Sem alteração",l=()=>{if(t.includes("mês")){const[c,p]=n.currentMonth.split("-");return new Date(c,p-1).toLocaleDateString("pt-BR",{month:"short",year:"numeric"})}else return n.currentMonth.split("-")[0]},i=()=>{if(t.includes("mês")){const[c,p]=n.currentMonth.split("-");return new Date(c,p-2).toLocaleDateString("pt-BR",{month:"short",year:"numeric"})}else return(parseInt(n.currentMonth.split("-")[0])-1).toString()};return`
    ...
    `:"";a.innerHTML=`
        <div class="cards-report-container">
            ...
            <div class="cards-grid">
                ${e.cards&&e.cards.length>0?e.cards.map(s=>{const r=Ae(s.cor,"#E67E22");return`
                    <div class="card-item surface-card surface-card--interactive surface-card--clip ${s.status_saude.status}"
                            ...
                            <div class="card-quick-info">
                                ...
                                ${s.proximos_meses&&s.proximos_meses.length>0&&s.proximos_meses.some(o=>o.valor>0)?`
                                    <div class="quick-info-item">
                                        ...
                                        <span>Próximo: ${v(s.proximos_meses.find(o=>o.valor>0)?.valor||0)}</span>
                                    </div>
                                `:""}
                            </div>
                        `:""}
                        ...
                        <div class="card-footer">
                            <button class="card-action-btn primary full-width" data-action="open-card-detail" data-card-id="${s.id||""}" data-card-nome="${b(s.nome)}" data-card-cor="${r}" data-card-month="${n.currentMonth}" title="Ver relatório detalhado">
                                <i data-lucide="eye"></i>
