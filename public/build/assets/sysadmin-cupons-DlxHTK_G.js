import{k as v,e as c,i as _,p as x}from"./api-DpYnTMaG.js";import{e as r}from"./utils-Bj4jxwhy.js";function l(){return"api/v1/cupons"}function I(){return`${l()}/estatisticas`}let d=[];document.addEventListener("DOMContentLoaded",()=>{window.LK?.modalSystem?.prepareOverlay("#modalCupom",{scope:"page"}),p()});async function p(){try{const t=await v(l());if(t.success)d=t.data.cupons,$();else throw new Error(t.message||"Erro ao carregar cupons")}catch(t){console.error("Erro ao carregar cupons:",t),LKFeedback.error(c(t,"Erro ao carregar cupons."))}finally{document.getElementById("loading").style.display="none"}}function $(){const t=document.getElementById("cuponsTableBody"),e=document.getElementById("cuponsTable"),s=document.getElementById("emptyState"),a=document.getElementById("cuponsStats");if(d.length===0){e.style.display="none",s.style.display="block",a.style.display="none";return}const i=d.filter(o=>o.is_valid).length,n=d.reduce((o,u)=>o+u.uso_atual,0);document.getElementById("statTotalCupons").textContent=d.length,document.getElementById("statCuponsAtivos").textContent=i,document.getElementById("statTotalUsos").textContent=n,a.style.display="grid",e.style.display="table",s.style.display="none",t.innerHTML=d.map(o=>{const u=o.is_valid?'<span class="badge badge-ativo"><i data-lucide="circle-check"></i> Válido</span>':'<span class="badge badge-inativo"><i data-lucide="x-circle"></i> Inválido</span>',E=o.tipo_desconto==="percentual"?'<span class="badge badge-percentual"><i data-lucide="percent"></i> Percentual</span>':'<span class="badge badge-fixo"><i data-lucide="dollar-sign"></i> Fixo</span>';let m="";if(o.limite_uso>0){const g=o.uso_atual/o.limite_uso*100;m=`<span class="uso-badge ${g>=80?"esgotado":g>=50?"limitado":""}"><i data-lucide="pie-chart"></i> ${o.uso_atual}/${o.limite_uso}</span>`}else m=`<span class="uso-badge"><i data-lucide="infinity"></i> ${o.uso_atual} usos</span>`;return`
            <tr>
                ...
                <td>
                    ...
                    <button class="btn-action btn-ver" data-action="verEstatisticas" data-cupom-id="${o.id}" title="Ver estatísticas">
        ...
        `,width:400,showConfirmButton:!0,confirmButtonText:"Fechar",confirmButtonColor:"#e67e22"})}document.addEventListener("click",t=>{const e=t.target.closest("[data-action]");if(!e)return;const s=e.dataset.action,a=e.dataset.cupomId?parseInt(e.dataset.cupomId,10):null,i=e.dataset.cupomCodigo||"";switch(s){case"abrirModalCriarCupom":B();break;case"fecharModalCupom":f();break;case"verDetalhesMobile":k(a);break;case"verEstatisticas":b(a);break;case"excluirCupom":y(a,i);break;case"verEstatisticasFromSwal":Swal.close(),b(a);break;case"excluirCupomFromSwal":Swal.close(),y(a,i);break}});document.addEventListener("change",t=>{const e=t.target.closest("[data-action]");if(e)switch(e.dataset.action){case"atualizarPlaceholder":C();break;case"toggleReativacao":w();break;case"toggleMesesInatividade":h();break}});
