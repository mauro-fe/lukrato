import{b as _,a as x,e as C}from"./api-CiEmwEpk.js";import{e as m}from"./utils-Bj4jxwhy.js";function s(a){return new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format(Number(a)||0)}function p(a,e=""){try{return(getComputedStyle(document.documentElement).getPropertyValue(a)||"").trim()||e}catch{return e}}const k={"check-circle":"circle-check","exclamation-triangle":"triangle-alert","arrow-trend-up":"trending-up","arrow-trend-down":"trending-down","info-circle":"info","times-circle":"x-circle",lightbulb:"lightbulb","chart-line":"line-chart"},h=a=>{const e=a.replace(/^fa-/,"");return k[e]||e};function g(a){return!a||a.length===0?'<div class="empty-message"><i data-lucide="inbox"></i><p>Nenhum lançamento neste mês</p></div>':a.map(e=>`
    ...
    `}function E(a){return a.map(e=>{const o=(e.total_parcelas-e.parcelas_restantes)/e.total_parcelas*100;return`
        ...
        <div class="parcelamentos-table-wrapper surface-card">${S(a.ativos)}</div>
