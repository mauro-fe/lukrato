...
`)},setupLimiteMoneyMask(){const e=document.getElementById("limiteTotal");if(!e){console.error("❌ Campo limiteTotal NÃO encontrado!");return}e.addEventListener("input",function(a){let t=a.target.value;t=t.replace(/[^\d]/g,"");const n=((parseInt(t)||0)/100).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".");a.target.value=n}),e.value="0,00"}};function ae(e){return typeof e!="string"?e:e.startsWith(v.BASE_URL)?e.slice(v.BASE_URL.length):e}async function $(e,{method:a="GET",data:t=null,headers:o={},timeout:r=15e3}={}){return J(ae(e),{method:a,headers:o,body:t},{timeout:r})}function te(e){if(l.previewMeta=e?.is_demo?e:null,l.previewMeta){window.LKDemoPreviewBanner?.show(l.previewMeta);return}window.LKDemoPreviewBanner?.hide()}const S={async loadCartoes(){const e=document.getElementById("cartoesGrid"),a=document.getElementById("emptyState"),t=document.getElementById("cartoesContainer");if(!(!e||!a))try{l.isLoading=!0,e.setAttribute("aria-busy","true"),t?.setAttribute("aria-busy","true"),e.innerHTML=`
            ...
            `,I()}finally{l.isLoading=!1,e.setAttribute("aria-busy","false"),t?.setAttribute("aria-busy","false")}},async verificarFaturasPendentes(){l.cartoes.forEach(a=>{a.temFaturaPendente=!1});const e=l.cartoes.map(async a=>{try{const t=await $(`${v.API_URL}/cartoes/${a.id}/faturas-pendentes`),o=w(t,{}),r=o?.meses||w(o,[])||[];a.temFaturaPendente=Array.isArray(r)&&r.length>0}catch{a.temFaturaPendente=!1}});await Promise.all(e)},async carregarAlertas(){try{let e;if(window.lkFetch){const a=await window.lkFetch.get(`${v.API_URL}/cartoes/alertas`,{timeout:1e4,maxRetries:1,showLoading:!1});e=w(a,{});const t=w(e,{});l.alertas=t?.alertas||[]}else{e=await $(`${v.API_URL}/cartoes/alertas`,{timeout:1e4});const a=w(e,{});l.alertas=a?.alertas||[]}S.renderAlertas()}catch(e){G("[Cartoes] Erro ao carregar alertas",e,"Erro ao carregar alertas"),l.alertas=[];const a=document.getElementById("alertasContainer");a&&(a.style.display="none")}},renderAlertas(){const e=document.getElementById("alertasContainer");if(e){if(l.alertas.length===0){e.style.display="none";return}e.style.display="block",e.innerHTML=`
        ...
        `},dismissAlerta(e){const a=e.closest(".alerta-item");a&&(a.style.animation="slideOut 0.3s ease-out forwards",setTimeout(()=>{a.remove();const t=document.getElementById("alertasContainer");t&&t.querySelectorAll(".alerta-item").length===0&&(t.style.display="none")},300))},async loadContasSelect(){const e=document.getElementById("contaVinculada"),a=document.getElementById("contaVinculadaHelp"),t=document.getElementById("cartaoContaEmptyHint");if(!e){console.error("❌ Select contaVinculada não encontrado!");return}try{const o=`${v.API_URL}/contas?only_active=0&with_balances=1`,r=await $(o),n=w(r,{});let c=[];if(Array.isArray(n)?c=n:Array.isArray(n?.contas)&&(c=n.contas),c.length===0)return e.disabled=!0,e.innerHTML='<option value="">Nenhuma conta disponivel</option>',a&&(a.textContent="Crie uma conta antes de vincular um cartao."),t&&(t.hidden=!1),console.warn("⚠️ Nenhuma conta encontrada"),0;const u=c.map(i=>{const p=i.instituicao_financeira?.nome||i.instituicao?.nome||i.nome||"Sem instituição",C=s.escapeHtml(i.nome||"Conta sem nome"),y=s.escapeHtml(p),f=parseFloat(i.saldoAtual||i.saldo_atual||i.saldo||i.saldo_inicial||0),b=s.formatMoney(f);return`<option value="${i.id}">${C} - ${y} - ${b}</option>`}).join("");return e.disabled=!1,e.innerHTML='<option value="">Selecione a conta</option>'+u,a&&(a.textContent="Conta onde o pagamento da fatura sera debitado."),t&&(t.hidden=!0),c.length}catch(o){return P("[Cartoes] Erro ao carregar contas",o,"Erro ao carregar contas"),e.disabled=!0,e.innerHTML='<option value="">Erro ao carregar contas</option>',a&&(a.textContent="Nao foi possivel carregar as contas agora."),t&&(t.hidden=!1),0}},async saveCartao(){const e=document.getElementById("formCartao");if(!e.checkValidity()){e.reportValidity();return}const a=document.getElementById("cartaoId").value,t=!!a,o=document.querySelector('meta[name="csrf-token"]')?.content||document.querySelector('input[name="csrf_token"]')?.value||"",r=document.getElementById("limiteTotal").value,n=s.parseMoney(r),c=document.getElementById("cartaoLembreteAviso")?.value||"",u=document.getElementById("contaVinculada"),i=document.getElementById("cartaoCanalInapp"),p=document.getElementById("cartaoCanalEmail");if(u?.disabled){s.showToast("error","Crie uma conta antes de cadastrar um cartao.");return}if(c&&!i?.checked&&!p?.checked){s.showToast("error","Selecione pelo menos um canal para o lembrete.");return}const C={nome_cartao:document.getElementById("nomeCartao").value.trim(),conta_id:u?.value?parseInt(u.value,10):null,bandeira:document.getElementById("bandeira").value,ultimos_digitos:document.getElementById("ultimosDigitos").value.trim(),limite_total:n,dia_fechamento:document.getElementById("diaFechamento").value||null,dia_vencimento:document.getElementById("diaVencimento").value||null,lembrar_fatura_antes_segundos:c?parseInt(c):null,fatura_canal_inapp:c&&i?.checked?1:0,fatura_canal_email:c&&p?.checked?1:0,csrf_token:o};try{const y=t?`${v.API_URL}/cartoes/${a}`:`${v.API_URL}/cartoes`,f=await $(y,{method:t?"PUT":"POST",data:C}),b=w(f,null);b?.gamification?.achievements&&Array.isArray(b.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(b.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",t?"Cartão atualizado com sucesso!":"Cartão criado com sucesso!"),g.UI.closeModal(),await S.loadCartoes()}catch(y){P("[Cartoes] Erro ao salvar cartão",y,"Erro ao salvar cartão"),s.showToast("error",F(y,"Erro ao salvar cartao"))}},async editCartao(e){const a=l.cartoes.find(t=>t.id===e);if(a){if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para editar.");return}g.UI.openModal("edit",a)}},async arquivarCartao(e){const a=l.cartoes.find(o=>o.id===e);if(!a)return;if(a.is_demo){s.showToast("info","Esse cartao e apenas um exemplo. Crie um cartao real para arquivar ou editar.");return}if(await s.showConfirmDialog("Arquivar Cartão",`Tem certeza que deseja arquivar o cartão "${a.nome_cartao}"? Você poderá restaurá-lo depois na página de Cartões Arquivados.`,"Arquivar"))try{await $(`${v.API_URL}/cartoes/${e}/archive`,{method:"POST"}),s.showToast("success","Cartão arquivado com sucesso!"),S.loadCartoes()}catch(o){P("[Cartoes] Erro ao arquivar cartão",o,"Erro ao arquivar cartão"),s.showToast("error",F(o,"Erro ao arquivar cartao"))}},async deleteCartao(e){return S.arquivarCartao(e)},async carregarFatura(e,a,t){try{const o=await $(`${v.API_URL}/cartoes/${e}/fatura?mes=${a}&ano=${t}`);return w(o,{itens:[],total:0,pago:0,pendente:0})}catch(o){if(o?.status===404)return{itens:[],total:0,pago:0,pendente:0};throw new Error(F(o,"Erro ao carregar fatura"))}},async carregarParcelamentosResumo(e,a,t){const o=await $(`${v.API_URL}/cartoes/${e}/parcelamentos-resumo?mes=${a}&ano=${t}`);return w(o,null)},async carregarHistoricoFaturas(e,a=12){const t=await $(`${v.API_URL}/cartoes/${e}/faturas-historico?limite=${a}`);return w(t,null)},async pagarParcelasIndividuais(e,a){try{const t=Array.from(e).map(n=>parseInt(n.dataset.id)),o=a.cartao_id||a.cartao?.id;if(!o)throw new Error("ID do cartão não encontrado na fatura");const r=await $(`${v.API_URL}/cartoes/${o}/parcelas/pagar`,{method:"POST",data:{parcela_ids:t,mes:a.mes,ano:a.ano}});if(r?.success!==!1){s.showToast("success",r.message||"Parcelas pagas com sucesso!");const n=document.querySelector(".modal-fatura-overlay");n&&g.Fatura.fecharModalFatura(n),await S.loadCartoes()}else throw new Error(r.message||"Erro ao pagar parcelas")}catch(t){s.showToast("error",F(t,"Erro ao processar a operacao do cartao"))}},async desfazerPagamento(e,a,t){if((await Swal.fire({title:"Desfazer pagamento?",html:`
                <p>Esta ação irá:</p>
            ...
            `,icon:"warning",showCancelButton:!0,confirmButtonText:"Sim, desfazer",cancelButtonText:"Cancelar",confirmButtonColor:"#d33",reverseButtons:!0})).isConfirmed)try{const r=await $(`${v.API_URL}/cartoes/${e}/fatura/desfazer-pagamento`,{method:"POST",data:{mes:a,ano:t}});if(r.success){s.showToast("success",r.message);const n=document.querySelector(".modal-fatura-overlay");n&&g.Fatura.fecharModalFatura(n),await S.loadCartoes()}else throw new Error(r.message||"Erro ao desfazer pagamento")}catch(r){s.showToast("error",F(r,"Erro ao processar a operacao do cartao"))}},async desfazerPagamentoParcela(e){if((await Swal.fire({title:"Desfazer pagamento desta parcela?",html:`
                <p>Esta ação irá:</p>
            ...
            <article
                ...
                tabindex="0"
                role="button"
                aria-label="Abrir detalhes do cartao ${f}, ${T(r)} do limite usado"
            >
...
Esta ação criará um lançamento de despesa na conta vinculada e liberará o limite do cartão.`,"Sim, Pagar"))return;const t=document.querySelector(".btn-pagar-fatura"),o=t?t.innerHTML:"";try{t&&(t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="icon-spin"></i> Processando...',I(),t.style.opacity="0.6",t.style.cursor="not-allowed");const r=await Y(`${v.API_URL}/cartoes/${e.cartao.id}/fatura/pagar`,{mes:e.mes,ano:e.ano}),n=w(r,null);n?.gamification?.achievements&&Array.isArray(n.gamification.achievements)&&(typeof window.notifyMultipleAchievements=="function"?window.notifyMultipleAchievements(n.gamification.achievements):console.error("❌ notifyMultipleAchievements não está disponível")),s.showToast("success",`Fatura paga com sucesso! ${n?.itens_pagos??""} parcela(s) quitada(s).`);const c=document.querySelector(".modal-fatura-overlay");c&&m.fecharModalFatura(c),g.API.loadCartoes()}catch(r){console.error("❌ Erro ao pagar fatura:",r),t&&(t.disabled=!1,t.innerHTML=o,t.style.opacity="1",t.style.cursor="pointer"),s.showToast("error",F(r,"Erro ao pagar fatura"))}},criarConteudoModalFaturaPaga(e,a,t,o){const r=o||e.cartao_id||e.cartao?.id,n=(e.itens||[]).filter(i=>i.pago).length,c=e.cartao?.bandeira?s.getBrandIcon(e.cartao.bandeira):null,u=a?.data_pagamento||(e.itens||[]).find(i=>i.pago&&i.data_pagamento)?.data_pagamento||null;return`
        ...
        `},async navegarMes(e,a,t,o){let r=a+o,n=t;r>12?(r=1,n++):r<1&&(r=12,n--);try{const[c,u,i]=await Promise.all([B(`${v.API_URL}/cartoes/${e}/fatura`,{mes:r,ano:n}).catch(()=>null),B(`${v.API_URL}/cartoes/${e}/parcelamentos-resumo`,{mes:r,ano:n}).catch(()=>null),B(`${v.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).catch(()=>null)]);if(!c)throw new Error("Erro ao carregar fatura");const p=c.data||c;let C=null,y=null;u&&(C=u.data||u),i&&(y=i.data||i);const f=document.querySelector(".modal-fatura-container");if(f){const b=s.resolverCorCartao(p,e);f.style.setProperty("--card-accent",b);const h=m.criarConteudoModal(p,C,y,e);f.innerHTML=h,I(),f.querySelector(".btn-fechar-fatura")?.addEventListener("click",()=>{const L=document.querySelector(".modal-fatura-overlay");m.fecharModalFatura(L)}),f.querySelector(".btn-pagar-fatura")?.addEventListener("click",()=>{m.pagarParcelasSelecionadas(p)});const E=document.querySelector(".modal-fatura-overlay");requestAnimationFrame(()=>{m.setupParcelaSelection(E,p)})}}catch(c){console.error("❌ Erro ao navegar entre meses:",c),s.showToast("error",F(c,"Erro ao carregar fatura"))}},async toggleHistoricoFatura(e){try{const a=document.querySelector(".modal-fatura-container");if(!a)return;if(a.querySelector(".historico-faturas")){const o=new Date,r=o.getMonth()+1,n=o.getFullYear(),[c,u,i]=await Promise.all([g.API.carregarFatura(e,r,n),g.API.carregarParcelamentosResumo(e,r,n).catch(()=>null),B(`${v.API_URL}/cartoes/${e}/fatura/status`,{mes:r,ano:n}).then(C=>w(C,null)).catch(()=>null)]),p=m.criarConteudoModal(c,u,i,e);a.innerHTML=p,I(),m.adicionarEventListenersModal(c)}else{const o=await g.API.carregarHistoricoFaturas(e),r=m.criarConteudoHistorico(o,e);a.innerHTML=r,I(),m.adicionarEventListenersModal(null)}}catch(a){console.error("❌ Erro ao alternar histórico:",a),s.showToast("error","Erro ao carregar histórico")}},criarConteudoHistorico(e,a){return`
            <div class="modal-fatura-header">
