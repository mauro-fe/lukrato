...
(function () {
    ...
    function updateProgressSection(data) {
        ...
        if (!isSuccess) {
            console.warn('[PAGE] Resposta inválida:', data);
            return;
        }
    ...
    function updateAchievements(data) {
        ...
        if (!isSuccess || !data.data || !data.data.achievements) {
            console.warn('[PAGE] Dados de conquistas inválidos:', data);
            return;
        }
    ...
    function renderAchievements(achievements) {
        ...
        // Renderizar ícones Lucide nos cards
    ...
    function updatePointsHistory(data) {
        ...
        if (!isSuccess || !data.data) {
            console.warn('[PAGE] Dados de histórico inválidos:', data);
    ...
    function updateLeaderboard(data) {
        ...
        if (!isSuccess || !data.data || !data.data.leaderboard) {
            console.warn('[PAGE] Dados de ranking inválidos:', data);
            return;
        }
        ...
        elements.leaderboardContainer.innerHTML = `
            ...
            const rankIcon = user.position === 1 ? '<i data-lucide="medal" style="color:#fbbf24;"></i>' : user.position === 2 ? '<i data-lucide="medal" style="color:#94a3b8;"></i>' : user.position === 3 ? '<i data-lucide="medal" style="color:#d97706;"></i>' : '';
        ...
        // Renderizar ícones Lucide no leaderboard
        if (window.lucide) lucide.createIcons();
    }
    ...
    function getActionIcon(action) {
        const icons = {
            // Lançamentos
            'LAUNCH_CREATED': 'coins',
            'LAUNCH_EDITED': 'pencil',
            'LAUNCH_DELETED': 'trash-2',
            'CREATE_LANCAMENTO': 'coins',
            'FIRST_LAUNCH_DAY': 'sunrise',

            // Categorias
            'CREATE_CATEGORIA': 'tag',
            'CATEGORY_CREATED': 'tag',

            // Atividade
            'DAILY_LOGIN': 'hand',
            'DAILY_ACTIVITY': 'check-circle',
            'VIEW_REPORT': 'bar-chart-3',

            // Metas
            'CREATE_META': 'target',
            'META_ACHIEVED': 'trophy',

            // Meses
            'CLOSE_MONTH': 'calendar',
            'POSITIVE_MONTH': 'heart',
