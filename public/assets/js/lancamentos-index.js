...
(() => {
    ...
    // Conta label: prioriza nome da conta (fallback: instituicao)
    function getContaLabel(t) {
        ...
        const contaLabel = pick(
            t.conta_nome,
            t.conta && t.conta.nome,
            typeof t.conta === "string" ? t.conta : '',
            t.conta_instituicao,
            t.conta && t.conta.instituicao
        );
    
        if (!t.eh_transferencia) return contaLabel || '-';
    
        const origem = pick(
            t.conta_origem_nome,
            t.origem_nome,
            t.conta_origem && t.conta_origem.nome,
            t.conta_origem_instituicao,
            t.origem_instituicao,
        ...
        const destino = pick(
            ...
            t.destino_nome,
            t.conta_destino && t.conta_destino.nome,
            t.conta_destino_instituicao,
            t.destino_instituicao,
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function openTransferModal(fromAccountId = null) {
                ...
                if (!fromAccountId) {
                    ...
                    trOrigemNome && (trOrigemNome.value = c ? ((c.nome || '').trim() || (c.instituicao || '').trim() || '') : '');
            ...
            async function populateOrigemSelect(selectedId = '') {
                ...
                contas.forEach(c => {
                    ...
                    op.value = c.id;
                    const nome = (c.nome || '').trim(), inst = (c.instituicao || '').trim();
                    op.textContent = nome || inst || '-';
