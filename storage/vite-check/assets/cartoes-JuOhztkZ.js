...
`)},setupLimiteMoneyMask(){const e=document.getElementById("limiteTotal");if(!e){console.error("❌ Campo limiteTotal NÃO encontrado!");return}e.addEventListener("input",function(a){let t=a.target.value;t=t.replace(/[^\d]/g,"");const n=((parseInt(t)||0)/100).toFixed(2).replace(".",",").replace(/\B(?=(\d{3})+(?!\d))/g,".");a.target.value=n}),e.value="0,00"}};function z(e){return typeof e!="string"?e:e.startsWith(p.BASE_URL)?e.slice(p.BASE_URL.length):e}async function w(e,{method:a="GET",data:t=null,headers:o={},timeout:r=15e3}={}){return D(z(e),{method:a,headers:o,body:t},{timeout:r})}const $={async loadCartoes(){const e=document.getElementById("cartoesGrid"),a=document.getElementById("emptyState"),t=document.getElementById("cartoesContainer");if(!(!e||!a))try{c.isLoading=!0,e.setAttribute("aria-busy","true"),t?.setAttribute("aria-busy","true"),e.innerHTML=`
            ...
            `,I()}finally{c.isLoading=!1,e.setAttribute("aria-busy","false"),t?.setAttribute("aria-busy","false")}},async verificarFaturasPendentes(){c.cartoes.forEach(a=>{a.temFaturaPendente=!1});const e=c.cartoes.map(async a=>{try{const t=await w(`${p.API_URL}/cartoes/${a.id}/faturas-pendentes`),o=b(t,{}),r=o?.meses||b(o,[])||[];a.temFaturaPendente=Array.isArray(r)&&r.length>0}catch{a.temFaturaPendente=!1}});await Promise.all(e)},async carregarAlertas(){try{let e;if(window.lkFetch){const a=await window.lkFetch.get(`${p.API_URL}/cartoes/alertas`,{timeout:1e4,maxRetries:1,showLoading:!1});e=b(a,{});const t=b(e,{});c.alertas=t?.alertas||[]}else{e=await w(`${p.API_URL}/cartoes/alertas`,{timeout:1e4});const a=b(e,{});c.alertas=a?.alertas||[]}$.renderAlertas()}catch(e){R("[Cartoes] Erro ao carregar alertas",e,"Erro ao carregar alertas"),c.alertas=[];const a=document.getElementById("alertasContainer");a&&(a.style.display="none")}},renderAlertas(){const e=document.getElementById("alertasContainer");if(e){if(c.alertas.length===0){e.style.display="none";return}e.style.display="block",e.innerHTML=`
