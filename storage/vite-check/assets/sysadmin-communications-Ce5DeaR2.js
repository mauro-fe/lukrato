import { b as F, c as w, l as v, e as x, d as _ } from "./api-Dkfcp6ON.js"; import { e as c, d as M } from "./utils-Bj4jxwhy.js"; const f = F(), H = { "fa-bullhorn": "megaphone", "fa-bell": "bell", "fa-paper-plane": "send", "fa-envelope": "mail", "fa-crown": "crown", "fa-star": "star", "fa-gift": "gift", "fa-rocket": "rocket", "fa-tag": "tag", "fa-info-circle": "info", "fa-exclamation-triangle": "triangle-alert", "fa-check-circle": "circle-check", "fa-users": "users", "fa-chart-line": "line-chart" }; let B = 1, g = 1, S = !1, $ = 0, b = 0; function N(a) { return H[a] || String(a || "").replace("fa-", "") } function r() { typeof lucide < "u" && lucide.createIcons() } function R(a, t = 140) { const e = String(a || "").trim(); return e.length <= t ? e : `${e.slice(0, t - 1).trimEnd()}...` } function k(a) { const t = document.querySelector('.btn-refresh[data-action="loadCampaigns"]'); t && (t.disabled = a, t.classList.toggle("is-loading", a), t.innerHTML = a ? '<i data-lucide="loader-2" class="icon-spin"></i>' : '<i data-lucide="refresh-cw"></i>', r()) } function I() { const a = document.getElementById("scheduleEnabled"), t = document.getElementById("scheduleDateTimeGroup"), e = document.getElementById("scheduledAt"), s = document.getElementById("btnSend"); if (!a || !t || !e || !s) return; const i = a.checked; t.style.display = i ? "block" : "none", e.required = i, i || (e.value = ""), s.innerHTML = i ? '<i data-lucide="calendar-clock"></i> Agendar Campanha' : '<i data-lucide="send"></i> Enviar Campanha', r() } function E(a) { const t = document.getElementById("historySummary"); if (!t) return; const e = Number(a?.total || 0), s = Number(a?.current_page || 1), i = Number(a?.total_pages || 1); if (e <= 0) { t.textContent = "Acompanhe status, entrega e segmentacao das ultimas campanhas."; return } t.textContent = `${e.toLocaleString("pt-BR")} campanha(s) registradas • pagina ${s} de ${i}.` } async function y() { const a = document.getElementById("recipientCount"); if (!a) return; const t = ++$; a.textContent = "...", a.closest(".preview-count")?.classList.remove("is-error"); try { const e = await _(`${f}api/campaigns/preview`, { plan: document.getElementById("filterPlan")?.value || "", status: document.getElementById("filterStatus")?.value || "", days_inactive: document.getElementById("filterDaysInactive")?.value || "" }); if (t !== $) return; const s = e?.data ?? e; if (e?.success === !1) { a.textContent = "?", a.closest(".preview-count")?.classList.add("is-error"); return } a.textContent = Number(s?.count || 0).toLocaleString("pt-BR") } catch (e) { if (t !== $) return; v("[Communications] Erro ao carregar preview", e, "Falha ao calcular destinatarios"), a.textContent = "?", a.closest(".preview-count")?.classList.add("is-error") } } function D(a) {
    ...
    const t = document.getElementById("campaignsList"); if (t) {
        if (!a || a.length === 0) { D("Crie sua primeira campanha para se comunicar com seus usuarios."); return } t.innerHTML = a.map(e => {
            const s = e.status_badge || {}, i = e.status === "sending" ? " icon-spin" : "", n = R(e.message, 132); let d = ""; e.was_scheduled && e.sent_at ? d = `
                ...
                </div>`); let o = ""; if (e.send_email && (e.emails_sent > 0 || e.emails_failed > 0)) {
                const l = Number(e.emails_sent || 0) + Number(e.emails_failed || 0); o = `
...
} function A(a) { const t = document.getElementById("paginationContainer"), e = document.getElementById("pageInfo"), s = document.getElementById("btnPrevPage"), i = document.getElementById("btnNextPage"); if (!(!t || !e || !s || !i)) { if (g = Number(a?.total_pages || 1), g <= 1) { t.style.display = "none"; return } t.style.display = "flex", e.textContent = `Pagina ${a.current_page} de ${g}`, s.disabled = a.current_page <= 1, i.disabled = a.current_page >= g } } async function h(a = 1) {
    const t = document.getElementById("campaignsList"); if (!t) return; const e = ++b; k(!0), t.innerHTML = `
        <div class="lk-loading-state">
            <i data-lucide="loader-2" class="icon-spin"></i>
            <span>Carregando campanhas...</span>
        </div>
    `, r(); try {
        const s = await _(`${f}api/campaigns`, { page: a, per_page: 10 }); if (e !== b) return; if (s?.success === !1) {
            const n = s?.message || "Erro ao carregar campanhas."; t.innerHTML = `
                <div class="empty-state">
                    <i data-lucide="circle-alert"></i>
                    <span>${c(n)}</span>
                </div>
            `, E(null), r(); return
        } const i = s?.data ?? s; K(i?.campaigns || []), A(i?.pagination || null), E(i?.pagination || null), B = a
    } catch (s) {
        if (e !== b) return; v("[Communications] Erro ao carregar campanhas", s, "Falha ao carregar campanhas"), t.innerHTML = `
            <div class="empty-state">
                <i data-lucide="circle-alert"></i>
...
} function q(a) { const t = B + a; t >= 1 && t <= g && h(t) } async function O(a) {
    const t = document.getElementById("campaignDetailModal"), e = document.getElementById("campaignDetailBody"); if (!t || !e) return; const s = new bootstrap.Modal(t); e.innerHTML = `
        <div class="text-center py-4">
            <i data-lucide="loader-2" class="icon-spin"></i>
        </div>
    `, s.show(), r(); try {
        const i = await _(`${f}api/campaigns/${a}`); if (i?.success === !1) { e.innerHTML = '<div class="text-danger">Erro ao carregar detalhes</div>'; return } const n = i?.data ?? i; let d = `
            <div class="detail-timeline">
                ...
                </div>`; if (n.scheduled_at) {
            const l = ["scheduled", "sending", "sent", "partial"].includes(n.status); d += `
                ...
                </div>`), d += "</div>"; let o = ""; if (n.send_email && (n.emails_sent > 0 || n.emails_failed > 0)) {
            const l = Number(n.emails_sent || 0) + Number(n.emails_failed || 0), u = l > 0 ? Math.round(Number(n.emails_sent || 0) / l * 100) : 0; o = `
