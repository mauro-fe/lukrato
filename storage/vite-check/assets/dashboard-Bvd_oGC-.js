import{l as x,d as se,e as J,j as ve,i as ye,c as Z}from"./api-Dkfcp6ON.js";import{a as oe,i as ne}from"./api-store-OncwIV5O.js";import{e as T}from"./utils-Bj4jxwhy.js";const y={BASE_URL:(()=>{let e=document.querySelector('meta[name="base-url"]')?.content||"";if(!e){const t=location.pathname.match(/^(.*\/public\/)/);e=t?location.origin+t[1]:location.origin+"/"}if(e&&!/\/public\/?$/.test(e)){const t=location.pathname.match(/^(.*\/public\/)/);t&&(e=location.origin+t[1])}return e.replace(/\/?$/,"/")})(),TRANSACTIONS_LIMIT:5,CHART_MONTHS:6,ANIMATION_DELAY:300};y.API_URL=`${y.BASE_URL}api/`;const c={saldoValue:document.getElementById("saldoValue"),receitasValue:document.getElementById("receitasValue"),despesasValue:document.getElementById("despesasValue"),saldoMesValue:document.getElementById("saldoMesValue"),chartContainer:document.getElementById("evolutionChart"),chartLoading:document.getElementById("chartLoading"),tableBody:document.getElementById("transactionsTableBody"),table:document.getElementById("transactionsTable"),cardsContainer:document.getElementById("transactionsCards"),emptyState:document.getElementById("emptyState"),monthLabel:document.getElementById("currentMonthText"),streakDays:document.getElementById("streakDays"),badgesGrid:document.getElementById("badgesGrid"),userLevel:document.getElementById("userLevel"),totalLancamentos:document.getElementById("totalLancamentos"),totalCategorias:document.getElementById("totalCategorias"),mesesAtivos:document.getElementById("mesesAtivos"),pontosTotal:document.getElementById("pontosTotal")},v={chartInstance:null,currentMonth:null,isLoading:!1},h={money:s=>{try{return Number(s||0).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}catch{return"R$ 0,00"}},dateBR:s=>{if(!s)return"-";try{const t=String(s).split(/[T\s]/)[0].match(/^(\d{4})-(\d{2})-(\d{2})$/);return t?`${t[3]}/${t[2]}/${t[1]}`:"-"}catch{return"-"}},formatMonth:s=>{try{const[e,t]=String(s).split("-").map(Number);return new Date(e,t-1,1).toLocaleDateString("pt-BR",{month:"long",year:"numeric"})}catch{return"-"}},formatMonthShort:s=>{try{const[e,t]=String(s).split("-").map(Number);return new Date(e,t-1,1).toLocaleDateString("pt-BR",{month:"short"})}catch{return"-"}},getCurrentMonth:()=>window.LukratoHeader?.getMonth?.()||new Date().toISOString().slice(0,7),getPreviousMonths:(s,e)=>{const t=[],[a,o]=s.split("-").map(Number);for(let n=e-1;n>=0;n--){const r=new Date(a,o-1-n,1),i=r.getFullYear(),d=String(r.getMonth()+1).padStart(2,"0");t.push(`${i}-${d}`)}return t},getCssVar:(s,e="")=>{try{return(getComputedStyle(document.documentElement).getPropertyValue(s)||"").trim()||e}catch{return e}},isLightTheme:()=>{try{return(document.documentElement?.getAttribute("data-theme")||"dark")==="light"}catch{return!1}},getContaLabel:s=>{if(typeof s.conta=="string"&&s.conta.trim())return s.conta.trim();const e=s.conta_instituicao??s.conta_nome??s.conta?.instituicao??s.conta?.nome??null,t=s.conta_destino_instituicao??s.conta_destino_nome??s.conta_destino?.instituicao??s.conta_destino?.nome??null;return s.eh_transferencia&&(e||t)?`${e||"-"}${t||"-"}`:s.conta_label&&String(s.conta_label).trim()?String(s.conta_label).trim():e||"-"},getTipoClass:s=>{const e=String(s||"").toLowerCase();return e==="receita"?"receita":e.includes("despesa")?"despesa":e.includes("transferencia")?"transferencia":""},removeLoadingClass:()=>{setTimeout(()=>{document.querySelectorAll(".kpi-value.loading").forEach(s=>{s.classList.remove("loading")})},y.ANIMATION_DELAY)}},we=()=>{const s=(document.documentElement.getAttribute("data-theme")||"").toLowerCase()==="light"||h.isLightTheme?.();return{isLightTheme:s,axisColor:s?h.getCssVar("--color-primary","#e67e22")||"#e67e22":"rgba(255, 255, 255, 0.6)",yTickColor:s?"#000":"#fff",xTickColor:s?h.getCssVar("--color-text-muted","#6c757d")||"#6c757d":"rgba(255, 255, 255, 0.6)",gridColor:s?"rgba(0, 0, 0, 0.08)":"rgba(255, 255, 255, 0.05)",tooltipBg:s?"rgba(255, 255, 255, 0.92)":"rgba(0, 0, 0, 0.85)",tooltipColor:s?"#0f172a":"#f8fafc",labelColor:s?"#0f172a":"#f8fafc"}},be=3e4,Ee=15e3,ie=window.__LK_CONFIG?.userId??"anon";function xe(s,e){return`dashboard:overview:${s}:${e}`}function R(s=h.getCurrentMonth(),{limit:e=y.TRANSACTIONS_LIMIT,force:t=!1}={}){return oe(`${y.API_URL}dashboard/overview`,{month:s,limit:e},{cacheKey:xe(s,e),ttlMs:be,force:t})}function k(s=null){const e=s?`dashboard:overview:${s}:`:"dashboard:overview:";ne(e)}function ke({force:s=!1}={}){return oe(`${y.BASE_URL}api/onboarding/checklist`,{},{cacheKey:`dashboard:onboarding:checklist:${ie}`,ttlMs:Ee,force:s})}function Ce(){ne(`dashboard:onboarding:checklist:${ie}`)}class Ie{constructor(e="greetingContainer"){this.container=document.getElementById(e);const t=window.__LK_CONFIG?.username||"Usuario";this.userName=t.split(" ")[0],this._listeningDataChanged=!1}render(){if(!this.container)return;const e=this.getGreeting(),a=new Date().toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long"});this.container.innerHTML=`
    ...
    `,typeof window.lucide<"u"&&window.lucide.createIcons())}}window.DashboardGreeting=Ie;class Le{constructor(e="healthScoreContainer"){this.container=document.getElementById(e),this.healthScore=0,this.maxScore=100,this.animationDuration=1200}render(){if(!this.container)return;const e=339.29;this.container.innerHTML=`
    ...
    `,this.refreshIcons())}renderMetas(e){const t=document.getElementById("foMetas");if(!t)return;if(!e||e.total_metas===0){this.renderMetasEmpty();return}const a=e.proxima_concluir,o=Math.round(e.progresso_geral||0);if(!a){this.updateGoalsHeadline("Voce tem metas ativas, mas nenhuma esta proxima de concluir."),t.innerHTML=`
        <div class="fo-card-header">
      ...
      `,this.refreshIcons();return}const n=a.cor||"var(--color-primary)",r=Math.round(a.progresso||0),i=Math.max(Number(a.valor_alvo||0)-Number(a.valor_atual||0),0);this.updateGoalsHeadline(`Faltam ${this.money(i)} para alcancar sua meta.`),t.innerHTML=`
      <div class="fo-card-header">
        <a href="${this.baseURL}financas#metas" class="fo-card-title">
          <i data-lucide="target" style="width:16px;height:16px;"></i>
          Metas
      ...
      <div class="fo-meta-destaque">
        <div class="fo-meta-icon" style="color:${n}; background:${n}18;">
          <i data-lucide="${a.icone||"target"}" style="width:16px;height:16px;"></i>
        </div>
      ...
      <div class="fo-metas-summary">
        <div class="fo-metas-stat">
          <span class="fo-metas-stat-value">${this.money(i)}</span>
    ...
    `,document.body.appendChild(a),setTimeout(()=>{a.classList.add("show")},10),setTimeout(()=>{a.classList.remove("show"),setTimeout(()=>a.remove(),300)},3500),this.fireConfetti("medium",.5,.6)}showCelebrationToast(e){const{title:t="🎉 Parabéns!",subtitle:a="Você fez progresso!",icon:o="party-popper",duration:n=3e3}=e;window.LK?.toast&&window.LK.toast.success(`${t}
${a}`)}showCelebrationModal(e,t){typeof Swal>"u"||Swal.fire({title:e,text:t,icon:"success",confirmButtonText:"🎉 Incrível!",confirmButtonColor:"var(--color-primary)",allowOutsideClick:!1,didOpen:()=>{this.fireConfetti("extreme",.5,.2)}})}screenFlash(e="#10b981",t=.3,a=1){const o=document.createElement("div");o.style.cssText=`
      position: fixed;
      top: 0;
      left: 0;
      width: 100%;
      ...
      pointer-events: none;
      transition: none;
    `,document.body.appendChild(o),setTimeout(()=>{o.style.transition=`opacity ${a/2}ms ease-out`,o.style.opacity=t},10),setTimeout(()=>{o.style.transition=`opacity ${a/2}ms ease-in`,o.style.opacity="0"},a/2),setTimeout(()=>o.remove(),a)}fireConfetti(e="medium",t=.5,a=.5){if(typeof confetti!="function")return;const o={small:{particleCount:30,spread:40},medium:{particleCount:60,spread:60},large:{particleCount:100,spread:90},extreme:{particleCount:150,spread:120}},n=o[e]||o.medium;confetti({...n,origin:{x:t,y:a},gravity:.8,decay:.95,zIndex:99999})}}window.CelebrationSystem=ce;document.addEventListener("DOMContentLoaded",()=>{window.celebrationSystem||(window.celebrationSystem=new ce)});function Be(){const s=window.BASE_URL||window.__LK_CONFIG?.baseUrl||"/";["assets/css/pages/admin-dashboard/health-score.css","assets/css/pages/admin-dashboard/greeting.css","assets/css/pages/admin-dashboard/health-score-insights.css"].forEach(t=>{const a=s+t;if(!document.querySelector(`link[href="${a}"]`)){const o=document.createElement("link");o.rel="stylesheet",o.href=a,o.type="text/css",document.head.appendChild(o)}})}function Ae(){return new Promise(s=>{let e=0;const t=setInterval(()=>{window.HealthScoreWidget&&window.DashboardGreeting&&window.HealthScoreInsights&&window.FinanceOverview&&(clearInterval(t),s()),e++>50&&(clearInterval(t),s())},100)})}function P(s,e){const t=document.getElementById(s);return t||e()}async function Me(){Be(),await Ae(),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ee):ee()}function ee(){const s=document.querySelector(".modern-dashboard");if(s){if(typeof window.DashboardGreeting<"u"&&(P("greetingContainer",()=>{const t=document.createElement("div");return t.id="greetingContainer",s.insertBefore(t,s.firstChild),t}),new window.DashboardGreeting().render()),typeof window.HealthScoreWidget<"u"){const e=P("healthScoreContainer",()=>{const a=document.createElement("div");a.id="healthScoreContainer";const o=s.querySelector(".kpi-grid");return o?o.insertAdjacentElement("afterend",a):s.insertBefore(a,s.children[1]||null),a}),t=new window.HealthScoreWidget;t.render(),t.load(),typeof window.HealthScoreInsights<"u"&&(P("healthScoreInsights",()=>{const a=document.createElement("div");return a.id="healthScoreInsights",a.className="health-score-insights-section",e.insertAdjacentElement("afterend",a),a}),window.healthScoreInsights=new window.HealthScoreInsights)}if(typeof window.FinanceOverview<"u"){P("financeOverviewContainer",()=>{const t=document.createElement("div");t.id="financeOverviewContainer";const a=s.querySelector(".provisao-section");return a?a.insertAdjacentElement("afterend",t):s.appendChild(t),t});const e=new window.FinanceOverview;e.render(),e.load()}typeof window.lucide<"u"&&window.lucide.createIcons()}}Me();window.__LK_CONFIG?.baseUrl;const K="lk_dashboard_tour_completed",O=[{target:".lk-health-score",title:"Sua Saúde Financeira",content:"Este número mostra como estão suas finanças. Quanto mais alto, melhor!",position:"bottom",icon:"heart-pulse"},{target:".lk-kpi-cards",title:"Resumo Financeiro",content:"Veja seu saldo, receitas e despesas do mês em um só lugar.",position:"bottom",icon:"bar-chart-3"},{target:'#addTransactionBtn, .fab, [data-action="add-transaction"]',title:"Adicionar Lançamentos",content:"Use este botão para registrar suas receitas e despesas.",position:"top",icon:"plus-circle",highlight:!0},{target:'.sidebar .nav-item[href*="lancamentos"]',title:"Todos os Lançamentos",content:"Aqui você vê o histórico completo de tudo que entrou e saiu.",position:"right",icon:"layers"},{target:'.sidebar .nav-item[href*="relatorios"]',title:"Relatórios",content:"Descubra para onde vai seu dinheiro com gráficos detalhados.",position:"right",icon:"pie-chart"}];class le{constructor(){this.currentStep=0,this.isActive=!1,this.overlay=null,this.tooltip=null}shouldShowTour(){const e=new URLSearchParams(window.location.search).get("first_visit")==="1",t=window.__lkFirstVisit===!0,a=localStorage.getItem(K)==="true";return(e||t)&&!a}start(){if(this.isActive)return;if(!document.querySelector(O[0].target)){console.warn("[Tour] First target not found, skipping tour");return}this.isActive=!0,this.currentStep=0,this.createOverlay(),this.showStep(0),this.trackEvent("tour_started")}createOverlay(){this.overlay=document.createElement("div"),this.overlay.className="lk-tour-overlay",this.overlay.innerHTML=`
            <div class="lk-tour-backdrop"></div>
        `,document.body.appendChild(this.overlay),this.overlay.querySelector(".lk-tour-backdrop").addEventListener("click",()=>{this.askToSkip()})}showStep(e){if(e>=O.length){this.complete();return}const t=O[e],a=document.querySelector(t.target);if(!a){this.showStep(e+1);return}this.currentStep=e,this.tooltip&&this.tooltip.remove(),this.highlightElement(a,t.highlight),this.createTooltip(a,t,e),this.scrollToElement(a)}highlightElement(e,t=!1){document.querySelectorAll(".lk-tour-highlighted").forEach(n=>{n.classList.remove("lk-tour-highlighted")}),e.classList.add("lk-tour-highlighted"),t&&e.classList.add("lk-tour-pulse");const a=e.getBoundingClientRect(),o=this.overlay.querySelector(".lk-tour-spotlight")||document.createElement("div");o.className="lk-tour-spotlight",o.style.cssText=`
            position: fixed;
            top: ${a.top-8}px;
            left: ${a.left-8}px;
            width: ${a.width+16}px;
        ...
        `,o.parentElement||this.overlay.appendChild(o)}createTooltip(e,t,a){const o=e.getBoundingClientRect(),n=a===O.length-1;this.tooltip=document.createElement("div"),this.tooltip.className=`lk-tour-tooltip lk-tour-tooltip-${t.position}`,this.tooltip.innerHTML=`
    ...
    `,document.head.appendChild(s)}let L=null;function Ne(){return de(),L||(L=new le),setTimeout(()=>{L.shouldShowTour()&&L.start()},1500),L}function De(){L||(L=new le,de()),L.start()}function Oe(){localStorage.removeItem(K)}typeof window<"u"&&(window.__LK_TOUR__={start:De,reset:Oe});const b={getOverview:async(s,e={})=>{const t=await R(s,e);return ve(t,{})},fetch:async s=>{if(window.LK?.api){const t=await LK.api.get(s);if(!t.ok)throw new Error(t.message||"Erro na API");return t.data}const e=await se(s);if(e?.success===!1)throw new Error(J({data:e},"Erro na API"));return e?.data??e},getMetrics:async s=>(await b.getOverview(s)).metrics||{},getAccountsBalances:async s=>{const e=await b.getOverview(s);return Array.isArray(e.accounts_balances)?e.accounts_balances:[]},getTransactions:async(s,e)=>{const t=await b.getOverview(s,{limit:e});return Array.isArray(t.recent_transactions)?t.recent_transactions:[]},getChartData:async s=>{const e=await b.getOverview(s);return Array.isArray(e.chart)?e.chart:[]},deleteTransaction:async s=>{if(window.LK?.api){const t=await LK.api.delete(`${y.API_URL}lancamentos/${s}`);if(t.ok)return t.data;throw new Error(t.message||"Erro ao excluir")}const e=[{request:()=>ye(`${y.API_URL}lancamentos/${s}`)},{request:()=>Z(`${y.API_URL}lancamentos/${s}/delete`,{})},{request:()=>Z(`${y.API_URL}lancamentos/delete`,{id:s})}];for(const t of e)try{return await t.request()}catch(a){if(a?.status!==404)throw new Error(J(a,"Erro ao excluir"))}throw new Error("Endpoint de exclusão não encontrado.")}},$={ensureSwal:async()=>{window.Swal},toast:(s,e)=>{if(window.LK?.toast)return LK.toast[s]?.(e)||LK.toast.info(e);window.Swal?.fire({toast:!0,position:"top-end",timer:2500,timerProgressBar:!0,showConfirmButton:!1,icon:s,title:e})},loading:(s="Processando...")=>{if(window.LK?.loading)return LK.loading(s);window.Swal?.fire({title:s,didOpen:()=>window.Swal.showLoading(),allowOutsideClick:!1,showConfirmButton:!1})},close:()=>{if(window.LK?.hideLoading)return LK.hideLoading();window.Swal?.close()},confirm:async(s,e)=>window.LK?.confirm?LK.confirm({title:s,text:e,confirmText:"Sim, confirmar",danger:!0}):(await window.Swal?.fire({title:s,text:e,icon:"warning",showCancelButton:!0,confirmButtonText:"Sim, confirmar",cancelButtonText:"Cancelar",confirmButtonColor:"var(--color-danger)",cancelButtonColor:"var(--color-text-muted)"}))?.isConfirmed,error:(s,e)=>{if(window.LK?.toast)return LK.toast.error(e||s);window.Swal?.fire({icon:"error",title:s,text:e,confirmButtonColor:"var(--color-primary)"})}},w={updateMonthLabel:s=>{c.monthLabel&&(c.monthLabel.textContent=h.formatMonth(s))},toggleAlertsSection:()=>{const s=document.getElementById("dashboardAlertsSection"),e=document.getElementById("dashboardAlertsOverview"),t=document.getElementById("dashboardAlertsBudget");if(!s)return;const a=e&&e.innerHTML.trim()!=="",o=t&&t.innerHTML.trim()!=="";s.style.display=a||o?"block":"none"},setSignedState:(s,e,t)=>{const a=document.getElementById(s),o=document.getElementById(e);!a||!o||(a.classList.remove("is-positive","is-negative","income","expense"),o.classList.remove("is-positive","is-negative"),t>0?(a.classList.add("is-positive"),o.classList.add("is-positive")):t<0&&(a.classList.add("is-negative"),o.classList.add("is-negative")))},renderHeroNarrative:({saldo:s,receitas:e,despesas:t,resultado:a})=>{const o=document.getElementById("dashboardHeroStatus"),n=document.getElementById("dashboardHeroMessage");if(!(!o||!n)){if(o.className="dashboard-status-chip",n.className="dashboard-hero-message",t>e){o.classList.add("dashboard-status-chip--negative"),n.classList.add("dashboard-hero-message--negative"),o.textContent="Mes em alerta",n.textContent="Atencao: voce gastou mais do que ganhou neste mes.";return}if(a>0){o.classList.add("dashboard-status-chip--positive"),n.classList.add("dashboard-hero-message--positive"),o.textContent=s>=0?"Mes saudavel":"Recuperando o mes",n.textContent="Voce conseguiu guardar dinheiro esse mes.";return}if(a===0){o.classList.add("dashboard-status-chip--neutral"),o.textContent="Mes equilibrado",n.textContent="O que entrou foi praticamente o que saiu neste mes.";return}o.classList.add("dashboard-status-chip--negative"),n.classList.add("dashboard-hero-message--negative"),o.textContent="Mes em alerta",n.textContent="Seu saldo do mes ficou negativo e vale rever os gastos agora."}},renderOverviewAlerts:({receitas:s,despesas:e})=>{const t=document.getElementById("dashboardAlertsOverview");t&&(e>s?(t.innerHTML=`
            ...
            `,typeof window.lucide<"u"&&window.lucide.createIcons()):t.innerHTML="",w.toggleAlertsSection())},renderChartInsight:(s,e)=>{const t=document.getElementById("chartInsight");if(!t)return;if(!Array.isArray(e)||e.length===0||e.every(r=>Number(r)===0)){t.textContent="Seu historico aparece aqui conforme voce usa o Lukrato mais vezes.";return}let a=0;e.forEach((r,i)=>{Number(r)<Number(e[a])&&(a=i)});const o=s[a],n=Number(e[a]||0);if(n<0){t.textContent=`Seu pior mes foi ${h.formatMonth(o)} (${h.money(n)}).`;return}t.textContent=`Seu pior mes foi ${h.formatMonth(o)} e mesmo assim fechou em ${h.money(n)}.`},renderKPIs:async s=>{try{const[e,t]=await Promise.all([b.getMetrics(s),b.getAccountsBalances(s)]),a={receitasValue:e.receitas||0,despesasValue:e.despesas||0,saldoMesValue:e.resultado||0};Object.entries(a).forEach(([i,d])=>{const u=document.getElementById(i);u&&(u.textContent=h.money(d))});const o=Number(e.saldoAcumulado??e.saldo??0),n=(Array.isArray(t)?t:[]).reduce((i,d)=>{const u=typeof d.saldoAtual=="number"?d.saldoAtual:d.saldoInicial||0;return i+(isFinite(u)?Number(u):0)},0),r=Array.isArray(t)&&t.length>0?n:o;c.saldoValue&&(c.saldoValue.textContent=h.money(r)),w.setSignedState("saldoValue","saldoCard",r),w.setSignedState("saldoMesValue","saldoMesCard",Number(e.resultado||0)),w.renderHeroNarrative({saldo:r,receitas:Number(e.receitas||0),despesas:Number(e.despesas||0),resultado:Number(e.resultado||0)}),w.renderOverviewAlerts({receitas:Number(e.receitas||0),despesas:Number(e.despesas||0)}),h.removeLoadingClass()}catch(e){x("Erro ao renderizar KPIs",e,"Falha ao carregar indicadores"),["saldoValue","receitasValue","despesasValue","saldoMesValue"].forEach(t=>{const a=document.getElementById(t);a&&(a.textContent="R$ 0,00",a.classList.remove("loading"))})}},renderTable:async s=>{try{const e=await b.getTransactions(s,y.TRANSACTIONS_LIMIT);c.tableBody&&(c.tableBody.innerHTML=""),c.cardsContainer&&(c.cardsContainer.innerHTML="");const t=Array.isArray(e)&&e.length>0;c.emptyState&&(c.emptyState.style.display=t?"none":"block"),c.table&&(c.table.style.display=t?"table":"none"),c.cardsContainer&&(c.cardsContainer.style.display=t?"flex":"none"),t&&e.forEach(a=>{const o=String(a.tipo||"").toLowerCase(),n=h.getTipoClass(o),r=String(a.tipo||"").replace(/_/g," "),i=a.categoria_nome??(typeof a.categoria=="string"?a.categoria:a.categoria?.nome)??null,d=i?T(i):'<span class="categoria-empty">Sem categoria</span>',u=T(h.getContaLabel(a)),p=T(a.descricao||a.observacao||"--"),g=T(r),S=Number(a.valor)||0,A=h.dateBR(a.data),C=document.createElement("tr");if(C.setAttribute("data-id",a.id),C.innerHTML=`
            ...
            `,c.tableBody&&c.tableBody.appendChild(C),c.cardsContainer){const E=document.createElement("div");E.className="transaction-card",E.setAttribute("data-id",a.id),E.innerHTML=`
              ...
              `,c.cardsContainer.appendChild(E)}})}catch(e){x("Erro ao renderizar transações",e,"Falha ao carregar transações"),c.emptyState&&(c.emptyState.style.display="block"),c.table&&(c.table.style.display="none"),c.cardsContainer&&(c.cardsContainer.style.display="none")}},renderChart:async s=>{if(!(!c.chartContainer||typeof ApexCharts>"u")){c.chartLoading&&(c.chartLoading.style.display="flex");try{const e=h.getPreviousMonths(s,y.CHART_MONTHS),t=e.map(g=>h.formatMonthShort(g)),a=await b.getChartData(s),o=new Map(a.map(g=>[g.month,Number(g.resultado||0)])),n=e.map(g=>o.get(g)||0);w.renderChartInsight(e,n);const{xTickColor:r,yTickColor:i,gridColor:d,isLightTheme:u}=we(),p=u?"light":"dark";v.chartInstance&&(v.chartInstance.destroy(),v.chartInstance=null),v.chartInstance=new ApexCharts(c.chartContainer,{chart:{type:"area",height:300,width:"100%",toolbar:{show:!1},background:"transparent",fontFamily:"Inter, Arial, sans-serif"},series:[{name:"Resultado do Mês",data:n}],xaxis:{categories:t,labels:{style:{colors:r}},axisBorder:{show:!1},axisTicks:{show:!1}},yaxis:{labels:{style:{colors:i},formatter:g=>h.money(g)}},colors:["#E67E22"],stroke:{curve:"smooth",width:3},fill:{type:"gradient",gradient:{shadeIntensity:1,opacityFrom:.35,opacityTo:.05,stops:[0,100]}},markers:{size:5,colors:["#E67E22"],strokeColors:"#fff",strokeWidth:2,hover:{size:7}},grid:{borderColor:d,strokeDashArray:4,xaxis:{lines:{show:!1}}},tooltip:{theme:p,y:{formatter:g=>`Resultado: ${h.money(g)}`}},legend:{show:!1},dataLabels:{enabled:!1},theme:{mode:p}}),v.chartInstance.render()}catch(e){x("Erro ao renderizar gráfico",e,"Falha ao carregar gráfico")}finally{c.chartLoading&&setTimeout(()=>{c.chartLoading.style.display="none"},300)}}}},te={delete:async(s,e)=>{try{if(await $.ensureSwal(),!await $.confirm("Excluir lançamento?","Esta ação não pode ser desfeita."))return;$.loading("Excluindo..."),await b.deleteTransaction(Number(s)),$.close(),$.toast("success","Lançamento excluído com sucesso!"),e&&(e.style.opacity="0",e.style.transform="translateX(-20px)",setTimeout(()=>{e.remove(),c.tableBody.children.length===0&&(c.emptyState&&(c.emptyState.style.display="block"),c.table&&(c.table.style.display="none"))},300)),document.dispatchEvent(new CustomEvent("lukrato:data-changed",{detail:{resource:"transactions",action:"delete",id:Number(s)}}))}catch(t){console.error("Erro ao excluir lançamento:",t),await $.ensureSwal(),$.error("Erro",J(t,"Falha ao excluir lançamento"))}}},I={isProUser:null,checkProStatus:async()=>{try{const s=await b.getOverview(h.getCurrentMonth());I.isProUser=s?.plan?.is_pro===!0}catch{I.isProUser=!1}return I.isProUser},render:async s=>{const e=document.getElementById("provisaoSection");if(!e)return;await I.checkProStatus();const t=document.getElementById("provisaoProOverlay"),a=I.isProUser;e.classList.remove("is-locked"),t&&(t.style.display="none");try{const o=await b.getOverview(s);I.renderData(o.provisao||null,a)}catch(o){x("Erro ao carregar provisão",o,"Falha ao carregar previsão")}},renderData:(s,e=!0)=>{if(!s)return;const t=s.provisao||{},a=h.money,o=document.getElementById("provisaoTitle"),n=document.getElementById("provisaoHeadline");o&&(o.textContent=`Se continuar assim, voce termina o mes com ${a(t.saldo_projetado||0)}`),n&&(n.textContent=(t.saldo_projetado||0)>=0?"A previsao abaixo considera seu saldo atual, o que ainda vai entrar e o que ainda vai sair.":"A previsao indica aperto no fim do mes se o ritmo atual continuar.");const r=document.getElementById("provisaoProximosTitle"),i=document.getElementById("provisaoVerTodos");r&&(r.innerHTML=e?'<i data-lucide="clock"></i> Próximos Vencimentos':'<i data-lucide="credit-card"></i> Próximas Faturas'),i&&(i.href=e?`${window.BASE_URL||"/"}lancamentos`:`${window.BASE_URL||"/"}faturas`);const d=document.getElementById("provisaoPagar"),u=document.getElementById("provisaoReceber"),p=document.getElementById("provisaoProjetado"),g=document.getElementById("provisaoPagarCount"),S=document.getElementById("provisaoReceberCount"),A=document.getElementById("provisaoProjetadoLabel"),C=u?.closest(".provisao-card");if(d&&(d.textContent=a(t.a_pagar||0)),e?(u&&(u.textContent=a(t.a_receber||0)),C&&(C.style.opacity="1")):(u&&(u.textContent="R$ --"),C&&(C.style.opacity="0.5")),p&&(p.textContent=a(t.saldo_projetado||0),p.style.color=(t.saldo_projetado||0)>=0?"":"var(--color-danger)"),g){const m=t.count_pagar||0,l=t.count_faturas||0;if(e){let f=`${m} pendente${m!==1?"s":""}`;l>0&&(f+=` • ${l} fatura${l!==1?"s":""}`),g.textContent=f}else g.textContent=`${l} fatura${l!==1?"s":""}`}e?S&&(S.textContent=`${t.count_receber||0} pendente${(t.count_receber||0)!==1?"s":""}`):S&&(S.textContent="Pro"),A&&(A.textContent=`saldo atual: ${a(t.saldo_atual||0)}`);const E=s.vencidos||{},q=document.getElementById("provisaoAlertDespesas");if(q){const m=E.despesas||{};if(e&&(m.count||0)>0){q.style.display="flex";const l=document.getElementById("provisaoAlertDespesasCount"),f=document.getElementById("provisaoAlertDespesasTotal");l&&(l.textContent=m.count),f&&(f.textContent=a(m.total||0))}else q.style.display="none"}const j=document.getElementById("provisaoAlertReceitas");if(j){const m=E.receitas||{};if(e&&(m.count||0)>0){j.style.display="flex";const l=document.getElementById("provisaoAlertReceitasCount"),f=document.getElementById("provisaoAlertReceitasTotal");l&&(l.textContent=m.count),f&&(f.textContent=a(m.total||0))}else j.style.display="none"}const z=document.getElementById("provisaoAlertFaturas");if(z){const m=E.count_faturas||0;if(m>0){z.style.display="flex";const l=document.getElementById("provisaoAlertFaturasCount"),f=document.getElementById("provisaoAlertFaturasTotal");l&&(l.textContent=m),f&&(f.textContent=a(E.total_faturas||0))}else z.style.display="none"}const M=document.getElementById("provisaoProximosList"),H=document.getElementById("provisaoEmpty");let F=s.proximos||[];if(e||(F=F.filter(m=>m.is_fatura===!0)),M)if(F.length===0){if(M.innerHTML="",H){const m=H.querySelector("span");m&&(m.textContent=e?"Nenhum vencimento pendente":"Nenhuma fatura pendente"),M.appendChild(H),H.style.display="flex"}}else{M.innerHTML="";const m=new Date().toISOString().slice(0,10);F.forEach(l=>{const f=(l.tipo||"").toLowerCase(),V=l.is_fatura===!0,X=(l.data_pagamento||"").split(/[T\s]/)[0],he=X===m,me=I.formatDateShort(X);let _="";he&&(_+='<span class="provisao-item-badge vence-hoje">Hoje</span>'),V?(_+='<span class="provisao-item-badge fatura"><i data-lucide="credit-card"></i> Fatura</span>',l.cartao_ultimos_digitos&&(_+=`<span>****${l.cartao_ultimos_digitos}</span>`)):(l.eh_parcelado&&l.numero_parcelas>1&&(_+=`<span class="provisao-item-badge parcela">${l.parcela_atual}/${l.numero_parcelas}</span>`),l.recorrente&&(_+='<span class="provisao-item-badge recorrente">Recorrente</span>'),l.categoria&&(_+=`<span>${T(l.categoria)}</span>`));const Q=V?"fatura":f,D=document.createElement("div");D.className="provisao-item"+(V?" is-fatura":""),D.innerHTML=`
                            <div class="provisao-item-dot ${Q}"></div>
                            <div class="provisao-item-info">
                                <div class="provisao-item-titulo">${T(l.titulo||"Sem título")}</div>
                                <div class="provisao-item-meta">${_}</div>
                            </div>
                            <span class="provisao-item-valor ${Q}">${a(l.valor||0)}</span>
                            <span class="provisao-item-data">${me}</span>
                        `,V&&l.cartao_id&&(D.style.cursor="pointer",D.addEventListener("click",()=>{const pe=(l.data_pagamento||"").split(/[T\s]/)[0],[ge,fe]=pe.split("-");window.location.href=`${window.BASE_URL||"/"}faturas?cartao_id=${l.cartao_id}&mes=${parseInt(fe)}&ano=${ge}`})),M.appendChild(D)})}const Y=document.getElementById("provisaoParcelas"),N=s.parcelas||{};if(Y)if(e&&(N.ativas||0)>0){Y.style.display="flex";const m=document.getElementById("provisaoParcelasText"),l=document.getElementById("provisaoParcelasValor");m&&(m.textContent=`${N.ativas} parcelamento${N.ativas!==1?"s":""} ativo${N.ativas!==1?"s":""}`),l&&(l.textContent=`${a(N.total_mensal||0)}/mês`)}else Y.style.display="none"},formatDateShort:s=>{if(!s)return"-";try{const e=s.match(/^(\d{4})-(\d{2})-(\d{2})$/);return e?`${e[3]}/${e[2]}`:"-"}catch{return"-"}}},B={refresh:async({force:s=!1}={})=>{if(v.isLoading)return;v.isLoading=!0;const e=h.getCurrentMonth();v.currentMonth=e,s&&k(e);try{w.updateMonthLabel(e),await Promise.allSettled([w.renderKPIs(e),w.renderTable(e),w.renderChart(e),I.render(e)])}catch(t){x("Erro ao atualizar dashboard",t,"Falha ao atualizar dashboard")}finally{v.isLoading=!1}},init:async()=>{await B.refresh({force:!1})}},Re={init:()=>{v.eventListenersInitialized||(v.eventListenersInitialized=!0,c.tableBody?.addEventListener("click",async s=>{const e=s.target.closest(".btn-del");if(!e)return;const t=s.target.closest("tr"),a=e.getAttribute("data-id");a&&(e.disabled=!0,await te.delete(a,t),e.disabled=!1)}),c.cardsContainer?.addEventListener("click",async s=>{const e=s.target.closest(".btn-del");if(!e)return;const t=s.target.closest(".transaction-card"),a=e.getAttribute("data-id");a&&(e.disabled=!0,await te.delete(a,t),e.disabled=!1)}),document.addEventListener("lukrato:data-changed",()=>{k(v.currentMonth||h.getCurrentMonth()),B.refresh({force:!1})}),document.addEventListener("lukrato:month-changed",()=>{B.refresh({force:!1})}),document.addEventListener("lukrato:theme-changed",()=>{w.renderChart(v.currentMonth||h.getCurrentMonth())}),Ne())}},ae=window.__LK_CONFIG?.baseUrl||window.BASE_URL||"/",He=`lk_user_${window.__LK_CONFIG?.userId??"anon"}_`;function G(s){return He+s}let U=null;function Fe(){if(window.__LK_ONBOARDING_CHECKLIST_INIT__)return;window.__LK_ONBOARDING_CHECKLIST_INIT__=!0;const s=!!window.__lkFirstVisit,e=G("lk_checklist_skipped"),t=G("lk_checklist_state"),a=document.getElementById("onboardingChecklist");if(!a||localStorage.getItem(e)==="1")return;U=localStorage.getItem(t)?JSON.parse(localStorage.getItem(t)):{};const o=document.getElementById("checklistDismiss");o&&o.addEventListener("click",n=>{n.preventDefault(),n.stopPropagation(),localStorage.setItem(e,"1"),ue(a)}),W(a,s),setInterval(()=>{W(a,s,{force:!0})},12e4),document.addEventListener("lukrato:data-changed",()=>{Ce(),setTimeout(()=>W(a,s,{force:!0}),500)})}function W(s,e,{force:t=!1}={}){ke({force:t}).then(a=>{if(!a.success)return;const o=a.data;if(o.all_complete&&(localStorage.setItem(G("lukrato_onboarding_completed"),"true"),!e)){ue(s);return}Ve(o),s.style.display="block",Ke(o),e&&Ge()}).catch(()=>{})}function Ve(s,e){const t=document.getElementById("checklistBadge"),a=document.getElementById("checklistProgressFill"),o=document.getElementById("checklistItems"),n=document.getElementById("checklistPrimaryLink");if(t&&(t.textContent=`${s.done_count}/${s.total}`),a){const u=s.total>0?s.done_count/s.total*100:0;a.style.width=`${u}%`}const r=s.items.filter(u=>!u.done).sort((u,p)=>u.priority-p.priority),i=r.slice(0,3),d=i[0]||r[0]||null;if(n&&d&&(n.href=ae+d.href),!!o){if(i.length===0){o.innerHTML=`
    ...
    `;return}o.innerHTML=i.map(u=>{const p=Pe(u);return`
      <a href="${ae}${u.href}" class="lk-onboarding-widget-item" data-item-key="${u.key}">
        <span class="lk-onboarding-widget-item-label">${p.label}</span>
        <span class="lk-onboarding-widget-item-desc">${p.description}</span>
      </a>
    `}).join("")}}function Pe(s){return{primeira_transacao:{label:"Registre seu primeiro gasto",description:"Comece a acompanhar o que entrou ou saiu."},segunda_transacao:{label:"Registre mais uma movimentacao",description:"Dois registros ja deixam o mes mais claro."},meta:{label:"Defina uma meta",description:"Escolha um objetivo para o dinheiro que sobra."},conta_conectada:{label:"Adicione outra conta ou cartao",description:"Tenha uma visao mais completa do seu dinheiro."},orcamento:{label:"Defina um limite de gastos",description:"Evite passar do ponto nas categorias principais."}}[s.key]||{label:s.label,description:s.description}}function Ke(s){const e={};s.items.forEach(t=>{e[t.key]=t.done}),U&&s.items.forEach(t=>{const a=!U[t.key],o=t.done;a&&o&&Ue(t)}),localStorage.setItem(G("lk_checklist_state"),JSON.stringify(e)),U=e}function Ue(s){window.LK?.toast&&window.LK.toast.success(`Item concluido: ${s.label}`),typeof confetti=="function"&&confetti({particleCount:40,spread:65,origin:{x:.5,y:.3}}),document.dispatchEvent(new CustomEvent("lukrato:checklist-item-completed",{detail:{key:s.key,label:s.label,points:s.points,icon:s.icon}}))}function Ge(){typeof confetti=="function"&&confetti({particleCount:60,spread:80,origin:{x:.5,y:.25}})}function ue(s){s.style.opacity="0",s.style.transform="translateY(-12px)",s.style.transition="all 0.25s ease",setTimeout(()=>{s.style.display="none"},250)}window.__LK_DASHBOARD_LOADER__||(window.__LK_DASHBOARD_LOADER__=!0,window.refreshDashboard=B.refresh,window.LK=window.LK||{},window.LK.refreshDashboard=B.refresh,(()=>{const e=()=>{Re.init(),B.init()};Fe(),document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})());
